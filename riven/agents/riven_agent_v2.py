...
from riven.adapters.riven_llm_adapter import rivenLLMAdapter
from riven.adapters.riven_llm_request_adapter import rivenLLMRequestAdapter
from riven.adapters.riven_llm_stream_adapter import rivenLLMStreamAdapter
from riven.agents.base_agent_v2 import BaseAgentV2
from riven.agents.helpers import (
...
from riven.constants import DEFAULT_MAX_STEPS, NON_USER_MSG_PREFIX, REQUEST_HEARTBEAT_PARAM
from riven.errors import ContextWindowExceededError, InsufficientCreditsError, LLMError
from riven.helpers import ToolRulesSolver
from riven.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns, ns_to_ms
from riven.helpers.reasoning_helper import scrub_inner_thoughts_from_messages
from riven.helpers.tool_execution_helper import enable_strict_mode
from riven.llm_api.llm_client import LLMClient
from riven.local_llm.constants import INNER_THOUGHTS_KWARG
from riven.log import get_logger
from riven.otel.tracing import log_event, trace_method, tracer
from riven.prompts.prompt_generator import PromptGenerator
from riven.schemas.agent import AgentState, UpdateAgent
from riven.schemas.enums import AgentType, LLMCallType, MessageStreamStatus, RunStatus, StepStatus
from riven.schemas.riven_message import rivenMessage, MessageType
from riven.schemas.riven_message_content import OmittedReasoningContent, ReasoningContent, RedactedReasoningContent, TextContent
from riven.schemas.riven_request import ClientSkillSchema, ClientToolSchema
from riven.schemas.riven_response import rivenResponse
from riven.schemas.riven_stop_reason import rivenStopReason, StopReasonType
from riven.schemas.message import Message, MessageCreate, MessageUpdate
from riven.schemas.openai.chat_completion_response import (
    FunctionCall,
    ToolCall,
    UsageStatistics,
...
from riven.schemas.step import Step, StepProgression
from riven.schemas.step_metrics import StepMetrics
from riven.schemas.tool import Tool
from riven.schemas.tool_execution_result import ToolExecutionResult
from riven.schemas.usage import rivenUsageStatistics
from riven.schemas.user import User
from riven.server.rest_api.utils import (
    create_approval_request_message_from_llm_response,
    create_riven_messages_from_llm_response,
)
...
from riven.services.credit_verification_service import CreditVerificationService
from riven.services.helpers.tool_parser_helper import runtime_override_tool_json_schema
...
from riven.services.summarizer.enums import SummarizationMode
from riven.services.summarizer.summarizer import Summarizer
...
from riven.settings import settings, summarizer_settings
from riven.system import package_function_response
from riven.types import JsonDict
from riven.utils import log_telemetry, safe_create_task, safe_create_task_with_return, united_diff, validate_function_response


class rivenAgentV2(BaseAgentV2):
    """
    Abstract base class for the riven agent loop, handling message management,
    ...
    async def build_request(
        ...
        response = self._step(
            ...
            llm_adapter=rivenLLMRequestAdapter(
    ...
    async def step(
        ...
        Returns:
            rivenResponse: Complete response with all messages and metadata
        """
        self._initialize_state()
        ...
        for i in range(max_steps):
            ...
            if credit_task is not None:
                if not await credit_task:
                    ...
                    self.stop_reason = rivenStopReason(stop_reason=StopReasonType.insufficient_credits)
                    break
                credit_task = None
            ...
            async for chunk in response:
                response_riven_messages.append(chunk)
        ...
        if not self.agent_state.message_buffer_autoclear:
            await self.summarize_conversation_history(
                ...
                new_riven_messages=self.response_messages,
        ...
        if self.stop_reason is None:
            self.stop_reason = rivenStopReason(stop_reason=StopReasonType.end_turn.value)

        result = rivenResponse(messages=response_riven_messages, stop_reason=self.stop_reason, usage=self.usage)
        if run_id:
    ...
    async def stream(
        ...
        if stream_tokens:
            llm_adapter = rivenLLMStreamAdapter(
                llm_client=self.llm_client,
                llm_config=self.agent_state.llm_config,
                call_type=LLMCallType.agent_step,
                agent_id=self.agent_state.id,
        ...
        try:
            ...
            if self.stop_reason is None:
                ...
                self.stop_reason = rivenStopReason(stop_reason=StopReasonType.max_steps.value)
        ...
        if run_id:
            riven_messages = Message.to_riven_messages_from_list(
            ...
            if not self.stop_reason:
                self.stop_reason = rivenStopReason(stop_reason=StopReasonType.end_turn.value)
            result = rivenResponse(messages=riven_messages, stop_reason=self.stop_reason, usage=self.usage)
            if self.job_update_metadata is None:
                self.job_update_metadata = {}
            self.job_update_metadata["result"] = result.model_dump(mode="json")
    ...
    async def _step(
    ...
    ) -> AsyncGenerator[rivenMessage | dict, None]:
        """
        ...
        Yields:
            rivenMessage or dict: Chunks for streaming mode, or request data for dry_run
        """
        ...
        try:
            ...
            if approval_request and approval_response:
                ...
                if run_id and await self._check_run_cancellation(run_id):
                    self.stop_reason = rivenStopReason(stop_reason=StopReasonType.cancelled.value)
                ...
                for llm_request_attempt in range(summarizer_settings.max_summarizer_retries + 1):
                    try:
                        ...
                        self.stop_reason = rivenStopReason(stop_reason=StopReasonType.invalid_llm_response.value)
                        raise e
                    except LLMError as e:
                        self.stop_reason = rivenStopReason(stop_reason=StopReasonType.llm_api_error.value)
                        raise e
            ...
            if tool_call is None and llm_adapter.tool_call is None:
                self.stop_reason = rivenStopReason(stop_reason=StopReasonType.no_tool_call.value)
            ...
            if llm_adapter.supports_token_streaming():
                if persisted_messages[-1].role != "approval":
                    tool_return = [msg for msg in persisted_messages if msg.role == "tool"][-1].to_riven_messages()[0]
            ...
            if not self.stop_reason:
                self.stop_reason = rivenStopReason(stop_reason=StopReasonType.error.value)
    ...
    def _initialize_state(self):
        ...
        self.usage = rivenUsageStatistics()
        self.last_step_usage: rivenUsageStatistics | None = None  # Per-step usage for Step token details
    ...
    async def _step_checkpoint_finish(
        ...
        if logged_step:
            ...
            # Build detailed token breakdowns from per-step rivenUsageStatistics
    ...
    def _update_global_usage_stats(self, step_usage_stats: rivenUsageStatistics):
    ...
    async def _handle_ai_response(
    ...
    ) -> tuple[list[Message], bool, rivenStopReason | None]:
        """
        ...
        if is_denial:
            ...
            stop_reason = None
            tool_call_messages = create_riven_messages_from_llm_response(
                agent_id=agent_state.id,
        ...
        if not is_approval and tool_rules_solver.is_requires_approval_tool(tool_call_name):
            ...
            stop_reason = rivenStopReason(stop_reason=StopReasonType.requires_approval.value)
        else:
    ...
    def _decide_continuation(
    ...
    ) -> tuple[bool, str | None, rivenStopReason | None]:
        continue_stepping = request_heartbeat
        heartbeat_reason: str | None = None
        stop_reason: rivenStopReason | None = None
        ...
        if tool_rule_violated:
            ...
            if tool_rules_solver.is_terminal_tool(tool_call_name):
                if continue_stepping:
                    stop_reason = rivenStopReason(stop_reason=StopReasonType.tool_rule.value)
                continue_stepping = False
        ...
        if is_final_step:
            continue_stepping = False
            stop_reason = rivenStopReason(stop_reason=StopReasonType.max_steps.value)
        else:
    ...
    async def summarize_conversation_history(
        self,
        in_context_messages: list[Message],
        new_riven_messages: list[Message],
        ...
        latest_messages = in_context_messages + new_riven_messages
            ...
            try:
                if force or (total_tokens and total_tokens > self.agent_state.llm_config.context_window):
                    ...
                    new_in_context_messages, _updated = await self.summarizer.summarize(
                        in_context_messages=in_context_messages,
                        new_riven_messages=new_riven_messages,
                        force=True,
                        clear=True,
                        run_id=run_id,
                        step_id=step_id,
                ...
                new_in_context_messages = in_context_messages + new_riven_messages
        else:
            new_in_context_messages = in_context_messages + new_riven_messages
    ...
    async def _log_request(
        ...
        if request_start_timestamp_ns:
            ...
            if request_span:
                request_span.add_event(name="riven_request_ms", attributes={"duration_ms": ns_to_ms(duration_ns)})
    ...
    def get_finish_chunks_for_stream(
        self,
        usage: rivenUsageStatistics,
        stop_reason: rivenStopReason | None = None,
    ):
        if stop_reason is None:
            stop_reason = rivenStopReason(stop_reason=StopReasonType.end_turn.value)
        return [
