...
from riven.agents.base_agent import BaseAgent
from riven.agents.ephemeral_summary_agent import EphemeralSummaryAgent
from riven.agents.helpers import (
    _build_rule_violation_result,
    _create_riven_response,
    _pop_heartbeat,
...
from riven.errors import ContextWindowExceededError, LLMError
from riven.helpers import ToolRulesSolver
from riven.helpers.datetime_helpers import AsyncTimer, get_utc_time, get_utc_timestamp_ns, ns_to_ms
...
from riven.interfaces.anthropic_streaming_interface import AnthropicStreamingInterface
from riven.interfaces.openai_streaming_interface import OpenAIStreamingInterface
...
from riven.otel.context import get_ctx_attributes
from riven.otel.metric_registry import MetricRegistry
from riven.otel.tracing import log_event, trace_method, tracer
from riven.schemas.agent import AgentState, UpdateAgent
from riven.schemas.enums import JobStatus, LLMCallType, ProviderType, StepStatus, ToolType
from riven.schemas.riven_message import MessageType
...
from riven.schemas.message import Message, MessageCreateBase
...
from riven.services.step_manager import NoopStepManager, StepManager
...
from riven.utils import log_telemetry, validate_function_response
...
class rivenAgent(BaseAgent):
    ...
    async def step_stream_no_tokens(
        ...
        for i in range(max_steps):
            if in_context_messages[-1].role == "approval":
                ...
                if await self._check_run_cancellation():
                    stop_reason = rivenStopReason(stop_reason=StopReasonType.cancelled.value)
                ...
                try:
                    ...
                    try:
                        ...
                        stop_reason = rivenStopReason(stop_reason=StopReasonType.invalid_llm_response.value)
                        raise e
                    ...
                    if not response.choices[0].message.tool_calls:
                        stop_reason = rivenStopReason(stop_reason=StopReasonType.no_tool_call.value)
                    ...
                    riven_messages = [m for m in riven_messages if m.message_type != "approval_response_message"]
                    ...
                    if not stop_reason:
                        stop_reason = rivenStopReason(stop_reason=StopReasonType.error.value)
        ...
        if not agent_state.message_buffer_autoclear:
            await self._rebuild_context_window(
                in_context_messages=current_in_context_messages,
                new_riven_messages=new_in_context_messages,
    ...
    async def _step(
        ...
        request_start_timestamp_ns: int | None = None,
        dry_run: bool = False,
    ) -> Union[tuple[list[Message], list[Message], rivenStopReason | None, rivenUsageStatistics], dict]:
        """
    ...
    async def step_stream(
        ...
        for i in range(max_steps):
            if in_context_messages[-1].role == "approval":
                ...
                # yields tool response as this is handled from riven and not the response from the LLM provider
                ...
                try:
                    ...
                    try:
                        ...
                        stop_reason = rivenStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
                        raise e
                    ...
                    persisted_messages, should_continue, stop_reason = await self._handle_ai_response(
                        ...
                        pre_computed_assistant_message_id=interface.riven_message_id,
                        step_id=effective_step_id,
                        initial_messages=initial_messages,
                        agent_step_span=agent_step_span,
                        is_final_step=(i == max_steps - 1),
                    ...
                    # Update step with actual usage now that we have it (if step was created)
                    if logged_step:
                        # Build detailed token breakdowns from rivenUsageStatistics
    ...
    async def summarize_conversation_history(self) -> None:
        ...
        new_in_context_messages, _updated = await self.summarizer.summarize(
            in_context_messages=in_context_messages, new_riven_messages=[], force=True
        )
    ...
    async def _create_llm_request_data_async(
        ...
        tools = [
            ...
            in {
                ToolType.CUSTOM,
                ToolType.riven_CORE,
                ToolType.riven_MEMORY_CORE,
                ToolType.riven_MULTI_AGENT_CORE,
                ToolType.riven_SLEEPTIME_CORE,
                ToolType.riven_VOICE_SLEEPTIME_CORE,
                ToolType.riven_BUILTIN,
                ToolType.riven_FILES_CORE,
