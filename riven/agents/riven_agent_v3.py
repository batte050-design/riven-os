...
from riven.adapters.sglang_native_adapter import SGLangNativeAdapter
from riven.adapters.simple_llm_request_adapter import SimpleLLMRequestAdapter
from riven.adapters.simple_llm_stream_adapter import SimpleLLMStreamAdapter
...
from riven.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns
...
from riven.otel.tracing import trace_method
from riven.schemas.agent import AgentState
from riven.schemas.enums import LLMCallType
from riven.schemas.riven_message import (
    ApprovalReturn,
    CompactionStats,
    EventMessage,
    rivenErrorMessage,
...
from riven.schemas.riven_response import rivenResponse, TurnTokenData
from riven.schemas.riven_stop_reason import rivenStopReason, StopReasonType
from riven.schemas.message import Message, MessageCreate, ToolReturn
from riven.schemas.openai.chat_completion_response import ChoiceLogprobs, ToolCall, ToolCallDenial, UsageStatistics
from riven.schemas.provider_trace import BillingContext
from riven.schemas.step import StepProgression
...
from riven.services.conversation_manager import ConversationManager
from riven.services.helpers.tool_parser_helper import runtime_override_tool_json_schema
from riven.services.llm_router import get_llm_routing_client
from riven.services.provider_manager import AUTO_MODE_HANDLES
from riven.services.summarizer.compact import compact_messages
from riven.services.summarizer.summarizer_config import CompactionSettings
from riven.services.summarizer.summarizer_sliding_window import count_tokens
from riven.services.summarizer.thresholds import get_compaction_trigger_threshold
from riven.settings import settings, summarizer_settings
from riven.system import package_function_response
from riven.utils import safe_create_task_with_return, validate_function_response
...
class rivenAgentV3(rivenAgentV2):
    """
    ...
    * Support Gemini / OpenAI client
    ...
    async def step(
        ...
        response_riven_messages = Message.to_riven_messages_from_list(
            self.response_messages,
        ...
        if include_return_message_types:
            response_riven_messages = [m for m in response_riven_messages if m.message_type in include_return_message_types]
        ...
        result = rivenResponse(
            messages=response_riven_messages,
    ...
    async def stream(
        ...
        try:
            ...
            for i in range(max_steps):
                ...
                async for chunk in response:
                    ...
                    if isinstance(chunk, rivenMessage) and (not chunk.id or not chunk.otid) and not is_compaction_event:
                        self.logger.warning(
            ...
            if self.stop_reason is None:
                ...
                if isinstance(e, SystemPromptTokenExceededError):
                    self.stop_reason = rivenStopReason(stop_reason=StopReasonType.context_window_overflow_in_system_prompt.value)
            ...
            if first_chunk:
                ...
                error_message = rivenErrorMessage(
                    run_id=run_id,
        ...
        try:
            ...
            if run_id:
                # Filter out rivenStopReason from messages (only valid in rivenStreamingResponse, not rivenResponse)
                filtered_messages = [m for m in response_riven_messages if not isinstance(m, rivenStopReason)]
                result = rivenResponse(
    ...
    def _create_summary_result_message(
        ...
        Returns:
            List of rivenMessage objects to yield to the client
        """
        if include_compaction_messages:
            ...
            messages = list(Message.to_riven_messages(summary_message))
        ...
        try:
            ...
            if approval_request and approval_response:
                ...
                if not tool_calls and not tool_call_denials and not tool_returns:
                    ...
                    self.should_continue = False
                    self.stop_reason = rivenStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
                    return
                    ...
                    try:
                        ...
                        if isinstance(e, ContextWindowExceededError) and llm_request_attempt < summarizer_settings.max_summarizer_retries:
                            ...
                            try:
                                ...
                                self.should_continue = False
                                self.stop_reason = rivenStopReason(
                ...
                if tool_calls:
                    ...
                    for tr in response_tool_returns:
                        ...
                        if tr.tool_call_id is None and tr.tool_returns:
                            continue
                        tool_return_riven = tr.to_riven_messages()[0]
                        if include_return_message_types is None or tool_return_riven.message_type in include_return_message_types:
                            yield tool_return_riven
            else:
    ...
    async def _handle_ai_response(
        ...
        if not tool_calls and not tool_call_denials and not tool_returns:
            ...
            if content is None or len(content) == 0:
                ...
                if uncalled:
                    ...
                    from riven.server.rest_api.utils import create_heartbeat_system_message
            ...
            # Case 1b: No tool call but has content
                ...
                assistant_message = create_riven_messages_from_llm_response(
                    agent_id=self.agent_state.id,
                    model=self.agent_state.llm_config.model,
                    function_name=None,
        ...
        if not is_approval_response:
            ...
            if requested_tool_calls:
                ...
                return messages_to_persist, False, rivenStopReason(stop_reason=StopReasonType.requires_approval.value)
        ...
        persisted_stop_reasons: list[rivenStopReason | None] = []
        ...
        for idx, spec in enumerate(exec_specs):
            ...
            if has_prefill_error:
                cont = False
                _hb_reason = None
                sr = rivenStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
            else:
    ...
    def _decide_continuation(
        ...
        if tool_call_name is None:
            ...
            if finish_reason == "length":
                return False, None, rivenStopReason(stop_reason=StopReasonType.max_tokens_exceeded.value)
            return False, None, rivenStopReason(stop_reason=StopReasonType.end_turn.value)
        else:
