...
from riven.agents.helpers import _create_riven_response, serialize_message_history
...
from riven.schemas.enums import MessageStreamStatus, ToolType
from riven.schemas.riven_message import LegacyrivenMessage, rivenMessage, MessageType
from riven.schemas.riven_response import rivenResponse
from riven.schemas.message import MessageCreate
from riven.schemas.tool_rule import ChildToolRule, ContinueToolRule, InitToolRule, TerminalToolRule
...
class VoiceSleeptimeAgent(rivenAgent):
    """
    A special variant of the rivenAgent that helps with offline memory computations specifically for voice.
    """
    ...
    async def step(
        ...
        new_in_context_messages, _updated = await self.summarizer.summarize(
            in_context_messages=current_in_context_messages, new_riven_messages=new_in_context_messages
        )
    ...
    async def _execute_tool(
        ...
        try:
            if target_tool.name == "rethink_user_memory" and target_tool.tool_type == ToolType.riven_VOICE_SLEEPTIME_CORE:
            ...
            elif target_tool.name == "finish_rethinking_memory" and target_tool.tool_type == ToolType.riven_VOICE_SLEEPTIME_CORE:
                return ToolExecutionResult(func_return="", status="success")
            elif target_tool.name == "store_memories" and target_tool.tool_type == ToolType.riven_VOICE_SLEEPTIME_CORE:
