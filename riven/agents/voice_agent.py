...
from riven.agents.voice_sleeptime_agent import VoiceSleeptimeAgent
from riven.constants import DEFAULT_MAX_STEPS, NON_USER_MSG_PREFIX, PRE_EXECUTION_MESSAGE_ARG, REQUEST_HEARTBEAT_PARAM
from riven.helpers.datetime_helpers import get_utc_time
from riven.helpers.tool_execution_helper import add_pre_execution_message, enable_strict_mode, remove_request_heartbeat
from riven.interfaces.openai_chat_completions_streaming_interface import OpenAIChatCompletionsStreamingInterface
...
from riven.schemas.enums import AgentType, MessageRole, ToolType
from riven.schemas.riven_response import rivenResponse
from riven.schemas.message import Message, MessageCreate
from riven.schemas.openai.chat_completion_request import (
    AssistantMessage,
...
from riven.server.rest_api.utils import (
    convert_in_context_riven_messages_to_openai,
...
from riven.settings import model_settings
...
class VoiceAgent(BaseAgent):
    ...
    async def step(self, input_messages: List[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> rivenResponse:
    ...
    async def step_stream(self, input_messages: List[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> AsyncGenerator[str, None]:
        ...
        if agent_state.llm_config.model_endpoint_type == "anthropic":
            ...
            raise ValueError("riven voice agents are only compatible with OpenAI or Anthropic.")
        ...
        riven_message_db_queue = await create_input_messages(
        ...
        for _ in range(max_steps):
            ...
            openai_messages = convert_in_context_riven_messages_to_openai(in_context_messages, exclude_system_messages=True)
        ...
        await self._rebuild_context_window(summarizer, in_context_messages, riven_message_db_queue)
    ...
    async def _handle_ai_response(
        ...
        riven_message_db_queue: List[Any],
    ) -> bool:
        """
        ...
        if streaming_interface.content_buffer:
            ...
            riven_message_db_queue.extend(assistant_msgs)
        ...
        if streaming_interface.tool_call_happened:
            ...
            riven_message_db_queue.extend(tool_call_messages)
    ...
    async def _rebuild_context_window(
        self, summarizer: Summarizer, in_context_messages: List[Message], riven_message_db_queue: List[Message]
    ) -> None:
        new_riven_messages = await self.message_manager.create_many_messages_async(riven_message_db_queue, actor=self.actor)

        # TODO: Make this more general and configurable, less brittle
        new_in_context_messages, _updated = await summarizer.summarize(
            in_context_messages=in_context_messages, new_riven_messages=new_riven_messages
        )
    ...
    def _build_tool_schemas(self, agent_state: AgentState, external_tools_only=True) -> List[Tool]:
        if external_tools_only:
            tools = [
                ...
                if t.tool_type in {ToolType.CUSTOM, ToolType.riven_FILES_CORE, ToolType.riven_BUILTIN, ToolType.EXTERNAL_MCP}
            ]
        else:
