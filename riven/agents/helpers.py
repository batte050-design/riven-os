...
from riven.errors import rivenError, PendingApprovalError
...
from riven.schemas.enums import MessageRole
...
from riven.schemas.message import ApprovalCreate, Message, MessageCreate, MessageCreateBase
...
from riven.server.rest_api.utils import create_approval_response_message_from_input, create_input_messages
from riven.services.message_manager import MessageManager

logger = get_logger(__name__)


def _create_riven_response(
    new_in_context_messages: list[Message],
    use_assistant_message: bool,
    usage: rivenUsageStatistics,
    stop_reason: Optional[rivenStopReason] = None,
    include_return_message_types: Optional[List[MessageType]] = None,
) -> rivenResponse:
    """
    Converts the newly created/persisted messages into a rivenResponse.
    """
    ...
    # Convert to riven messages first
    response_messages = Message.to_riven_messages_from_list(
    ...
    return rivenResponse(messages=response_messages, stop_reason=stop_reason, usage=usage)
...
def merge_and_validate_prefilled_args(tool: "Tool", llm_args: Dict[str, Any], prefilled_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    from riven.schemas.tool import Tool  # local import to avoid circulars in type hints
