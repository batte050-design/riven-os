...
from riven.agents.helpers import _prepare_in_context_messages_async
from riven.constants import DEFAULT_MAX_STEPS
...
from riven.jobs.types import RequestStatusUpdateInfo, StepStatusUpdateInfo
...
from riven.schemas.enums import AgentStepStatus, JobStatus, MessageStreamStatus, ProviderType, SandboxType, ToolType
from riven.schemas.job import JobUpdate
from riven.schemas.riven_message import LegacyrivenMessage, rivenMessage
...
from riven.schemas.riven_request import rivenBatchRequest
from riven.schemas.riven_response import rivenBatchResponse, rivenResponse
from riven.schemas.llm_batch_job import AgentStepState, LLMBatchItem
from riven.schemas.message import Message, MessageCreate
from riven.schemas.openai.chat_completion_response import ToolCall as OpenAIToolCall
from riven.schemas.sandbox_config import SandboxConfig
from riven.schemas.tool_execution_result import ToolExecutionResult
from riven.schemas.user import User
from riven.server.rest_api.utils import create_heartbeat_system_message, create_riven_messages_from_llm_response
...
class rivenAgentBatch(BaseAgent):
    def __init__(
        self,
    ...
    async def step_until_request(
        self,
        batch_requests: List[rivenBatchRequest],
        riven_batch_job_id: str,
        ...
        llm_batch_job = await self.batch_manager.create_llm_batch_job_async(
            ...
            riven_batch_job_id=riven_batch_job_id,
        )
        ...
        return rivenBatchResponse(
            riven_batch_id=llm_batch_job.riven_batch_job_id,
    ...
    @trace_method
    async def resume_step_after_request(self, riven_batch_id: str, llm_batch_id: str) -> rivenBatchResponse:
        ...
        if len(next_reqs) == 0:
            await self.job_manager.update_job_by_id_async(
                job_id=riven_batch_id, job_update=JobUpdate(status=JobStatus.completed), actor=self.actor
            )
            return rivenBatchResponse(
                riven_batch_id=llm_batch_job.riven_batch_job_id,
                last_llm_batch_id=llm_batch_job.id,
        ...
        return await self.step_until_request(
            ...
            riven_batch_job_id=riven_batch_id,
    ...
    async def _bulk_rethink_memory_async(self, params: List[ToolExecutionParams]) -> Sequence[tuple[str, ToolExecutionResult]]:
        ...
        for param in params:
            ...
            # TODO: Move 'rethink_memory' to a native riven tool that we control
    ...
    async def _prepare_next_iteration_async(
        ...
        ctx: _ResumeContext,
        msg_map: Dict[str, List[Message]],
    ) -> Tuple[List[rivenBatchRequest], Dict[str, AgentStepState]]:
        # who continues?
        ...
        batch_reqs: List[rivenBatchRequest] = []
    ...
    def _prepare_tools_per_agent(agent_state: AgentState, tool_rules_solver: ToolRulesSolver) -> List[dict]:
        tools = [t for t in agent_state.tools if t.tool_type in {ToolType.CUSTOM, ToolType.riven_CORE, ToolType.riven_MEMORY_CORE}]
    ...
    ) -> AsyncGenerator[Union[rivenMessage, LegacyrivenMessage, MessageStreamStatus], None]:
        raise NotImplementedError
