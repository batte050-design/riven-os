...
def get_function_name_and_docstring(source_code: str, name: Optional[str] = None) -> Tuple[str, str]:
    ...
    try:
        ...
        if not function_def:
            raise rivenToolCreateError("No function definition found in source code")
        ...
        if not function_name:
            raise rivenToolCreateError("Could not determine function name")
        ...
        traceback.print_exc()
        raise rivenToolCreateError(f"Failed to parse function name and docstring: {str(e)}")
