...
def derive_typescript_json_schema(source_code: str, name: Optional[str] = None) -> dict:
    ...
    Raises:
        rivenToolCreateError: If parsing fails or no exported function is found
    """
    try:
        ...
        if not match:
            raise rivenToolCreateError("No exported function found in TypeScript source code")
        ...
        raise rivenToolCreateError(f"TypeScript schema generation failed: {str(e)}") from e
