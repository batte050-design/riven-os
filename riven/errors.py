...
class rivenError(Exception):
    """Base class for all riven related errors."""
...
class PendingApprovalError(rivenError):
...
class NoActiveRunsToCancelError(rivenError):
...
class ConcurrentUpdateError(rivenError):
...
class ConversationBusyError(rivenError):
...
class MemoryRepoBusyError(rivenError):
...
class rivenToolCreateError(rivenError):
...
class rivenToolNameConflictError(rivenError):
...
class rivenToolNameSchemaMismatchError(rivenToolCreateError):
...
class rivenConfigurationError(rivenError):
...
class EmbeddingConfigRequiredError(rivenError):
...
class rivenAgentNotFoundError(rivenError):
    """Error raised when an agent is not found."""


class rivenUserNotFoundError(rivenError):
    """Error raised when a user is not found."""


class rivenUnsupportedFileUploadError(rivenError):
    """Error raised when an unsupported file upload is attempted."""


class rivenInvalidArgumentError(rivenError):
...
class rivenImageFetchError(rivenError):
...
class rivenMCPError(rivenError):
    """Base error for MCP-related issues."""


class rivenInvalidMCPSchemaError(rivenMCPError):
...
class rivenMCPConnectionError(rivenMCPError):
...
class rivenMCPTimeoutError(rivenMCPError):
...
class rivenServiceUnavailableError(rivenError):
...
class rivenUnexpectedStreamCancellationError(rivenError):
    """Error raised when a streaming request is terminated unexpectedly."""


class rivenExpiredError(rivenError):
...
class BedrockPermissionError(rivenError):
...
class LLMJSONParsingError(rivenError):
...
class LocalLLMError(rivenError):
...
class LocalLLMConnectionError(rivenError):
...
class ContextWindowExceededError(rivenError):
...
class RateLimitExceededError(rivenError):
...
class rivenMessageError(rivenError):
    """Base error class for handling message-related errors."""

    messages: List[Union["Message", "rivenMessage"]]
    default_error_message: str = "An error occurred with the message."

    def __init__(self, *, messages: List[Union["Message", "rivenMessage"]], explanation: Optional[str] = None) -> None:
        error_msg = self.construct_error_message(messages, self.default_error_message, explanation)
        super().__init__(error_msg)
        self.messages = messages

    @staticmethod
    def construct_error_message(messages: List[Union["Message", "rivenMessage"]], error_msg: str, explanation: Optional[str] = None) -> str:
...
class HandleNotFoundError(rivenError):
...
class InsufficientCreditsError(rivenError):
...
class RunCancelError(rivenError):
