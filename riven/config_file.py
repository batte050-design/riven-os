"""
riven Configuration File Support
...
This maps to environment variables:
    riven_TELEMETRY_ENABLE_DATADOG=true
    riven_PG_HOST=localhost
...
Config file locations (in order of precedence):
    1. ~/.riven/conf.yaml
    2. ./conf.yaml
    3. riven_CONFIG_PATH environment variable
"""
...
DEFAULT_USER_CONFIG = Path.home() / ".riven" / "conf.yaml"
...
def load_config_file(config_path: str | Path | None = None) -> dict[str, Any]:
    ...
    env_path = os.environ.get("riven_CONFIG_PATH")
...
def config_to_env_vars(config: dict[str, Any]) -> dict[str, str]:
    ...
    # Handle 'riven' section with riven_ prefix
    if "riven" in config:
        _flatten_with_prefix(config["riven"], "riven", env_vars)
