...
def _get_readiness_settings():
    from riven.settings import readiness_settings
...
class EventLoopWatchdog:
    ...
    def _maybe_degrade_readiness(self, now: float, current_lag_ms: float) -> None:
        ...
        try:
            ...
            from riven.monitoring.readiness_state import get_readiness_state, mark_degraded
    ...
    def _maybe_recover_readiness(self, now: float) -> None:
        ...
        try:
            ...
            from riven.monitoring.readiness_state import clear_degraded, get_readiness_state
        ...
        try:
            ...
            for task in active_tasks:
                try:
                    ...
                    # Find top riven frame for grouping
                    for frame in reversed(stack):
                        if "riven" in frame.f_code.co_filename:
                            idx = frame.f_code.co_filename.find("riven/")
            ...
            logger.warning(f"  riven tasks: {total_tasks} total")
            ...
            for i, (location, tasks) in enumerate(sorted_patterns, 1):
                ...
                if i <= 3:
                    ...
                    for frame in sample_stack[-8:]:
                        ...
                        riven_idx = filename.find("riven/")
                        if riven_idx != -1:
                            short_path = filename[riven_idx + 6 :]
