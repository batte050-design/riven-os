...
@dataclass
class rivenConfig:
    config_path: str = os.getenv("MEMGPT_CONFIG_PATH") or os.path.join(riven_DIR, "config")
    ...
    metadata_storage_path: str = riven_DIR
    ...
    riven_version: str = riven.__version__
    ...
    @classmethod
    def load(cls, llm_config: Optional[LLMConfig] = None, embedding_config: Optional[EmbeddingConfig] = None) -> "rivenConfig":
        # avoid circular import
        from riven.utils import printd

        # from riven.migrate import VERSION_CUTOFF, config_is_compatible
        # if not config_is_compatible(allow_empty=True):
        #    error_message = " ".join(
        #        [
        #            f"\nYour current config file is incompatible with riven versions later than {VERSION_CUTOFF}.",
        #            f"\nTo use riven, you must either downgrade your riven version (<= {VERSION_CUTOFF}) or regenerate your config using `riven configure`, or `riven migrate` if you would like to migrate old agents.",
        #        ]
        #    )
        ...
        if os.getenv("MEMGPT_CONFIG_PATH"):
            ...
            config_path = rivenConfig.config_path
        ...
        if os.path.exists(config_path):
            ...
            config_dict = {
                ...
                "riven_version": get_field(config, "version", "riven_version"),
            }
    ...
    def save(self):
        ...
        # set version
        set_field(config, "version", "riven_version", riven.__version__)
    ...
    def create_config_dir():
        if not os.path.exists(riven_DIR):
        ...
        for folder in folders:
            if not os.path.exists(os.path.join(riven_DIR, folder)):
                os.makedirs(os.path.join(riven_DIR, folder))
