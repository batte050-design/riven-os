...
from riven.otel.tracing import log_attributes, trace_method
from riven.schemas.openai.chat_completion_response import ChatCompletionResponse
from riven.server.rest_api.dependencies import HeaderParams
...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from riven.openai_backcompat.openai_object import OpenAIObject
...
def validate_function_response(
    ...
    """Check to make sure that a function used by riven returned a valid response. Truncates to return_char_limit if necessary.
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(riven_DIR, "agents")
...
def list_human_files():
    ...
    defaults_dir = os.path.join(riven.__path__[0], "humans", "examples")
    user_dir = os.path.join(riven_DIR, "humans")

    riven_defaults = os.listdir(defaults_dir)
    riven_defaults = [os.path.join(defaults_dir, f) for f in riven_defaults if f.endswith(".txt")]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return riven_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(riven.__path__[0], "personas", "examples")
    user_dir = os.path.join(riven_DIR, "personas")
...
def get_friendly_error_msg(function_name: str, exception_name: str, exception_message: str):
    from riven.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
...
def log_telemetry(logger: Logger, event: str, **kwargs):
    ...
    """
    from riven.settings import log_settings
...
def is_1_0_sdk_version(headers: HeaderParams):
    ...
    2. If user_agent is provided and in the format
        @riven-ai/riven-client/version (node) or
        riven-client/version (python)
    """
    from riven.log import get_logger
    ...
    if len(parts) == 3:
        # Format: @riven-ai/riven-client/version
        if parts[0] != "@riven-ai" or parts[1] != "riven-client":
            return False
    elif len(parts) == 2:
        # Format: riven-client/version
        if parts[0] != "riven-client":
            return False
    else:
        return False
...
async def decrypt_agent_secrets(agents: list) -> list:
    ...
    async def decrypt_env_var(env_var):
        from riven.orm.agent import ENCRYPTED_PLACEHOLDER
