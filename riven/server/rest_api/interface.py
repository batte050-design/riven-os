...
from riven.helpers.datetime_helpers import is_utc_datetime
from riven.interface import AgentInterface
from riven.local_llm.constants import INNER_THOUGHTS_KWARG
from riven.schemas.enums import MessageStreamStatus
...
from riven.schemas.riven_message_content import ReasoningContent, RedactedReasoningContent, TextContent
from riven.schemas.message import Message
from riven.schemas.openai.chat_completion_response import ChatCompletionChunkResponse
...
from riven.streaming_utils import FunctionArgumentsStreamHandler, JSONInnerThoughtsExtractor
from riven.utils import parse_json
...
class StreamingServerInterface(AgentChunkStreamingInterface):
    ...
    async def _create_generator(self) -> AsyncGenerator[Union[rivenMessage, LegacyrivenMessage, MessageStreamStatus], None]:
    ...
    def _push_to_buffer(
        ...
        assert isinstance(item, rivenMessage) or isinstance(item, LegacyrivenMessage) or isinstance(item, MessageStreamStatus), (
    ...
    def _process_chunk_to_riven_style(
        self,
                    ...
                    # If we have main_json, we should output a ToolCallMessage
                        ...
                        # output the arguments chunk as a ToolCallMessage
                            ...
                            if self.use_assistant_message and (
                                ...
                                from riven.streaming_utils import sanitize_streamed_message_content
    ...
    def _process_chunk_to_openai_style(self, chunk: ChatCompletionChunkResponse) -> Optional[dict]:
        """Chunks should look like OpenAI, but be remapped from riven-style concepts.
    ...
    def process_chunk(
        ...
        if self.streaming_chat_completion_mode:
            ...
            processed_chunk = self._process_chunk_to_riven_style(
                chunk=chunk,
