...
from riven.__init__ import __version__ as riven_version
from riven.agents.exceptions import IncompatibleAgentType
from riven.constants import ADMIN_PREFIX, API_PREFIX
from riven.errors import (
    ...
    HandleNotFoundError,
    rivenAgentNotFoundError,
    rivenExpiredError,
    rivenImageFetchError,
    rivenInvalidArgumentError,
    rivenInvalidMCPSchemaError,
    rivenMCPConnectionError,
    rivenMCPTimeoutError,
    rivenServiceUnavailableError,
    rivenToolCreateError,
    rivenToolNameConflictError,
    rivenUnsupportedFileUploadError,
    rivenUserNotFoundError,
...
from riven.helpers.json_helpers import sanitize_unicode_surrogates
from riven.helpers.pinecone_utils import get_pinecone_indices, should_use_pinecone, upsert_pinecone_indices
from riven.jobs.scheduler import start_scheduler_with_leader_election
from riven.log import get_logger
from riven.orm.errors import (
...
from riven.otel.tracing import get_trace_id
from riven.schemas.riven_message import create_riven_error_message_schema, create_riven_message_union_schema
from riven.schemas.riven_message_content import (
    create_riven_assistant_message_content_union_schema,
    create_riven_message_content_union_schema,
    create_riven_tool_return_content_union_schema,
    create_riven_user_message_content_union_schema,
)
from riven.server.constants import REST_DEFAULT_PORT
...
from riven.server.global_exception_handler import setup_global_exception_handlers

# NOTE(charles): these are extra routes that are not part of v1 but we still need to mount to pass tests
from riven.server.rest_api.auth.index import setup_auth_router  # TODO: probably remove right?
from riven.server.rest_api.interface import StreamingServerInterface
from riven.server.rest_api.middleware import CheckPasswordMiddleware, LoggingMiddleware, RequestIdMiddleware
from riven.server.rest_api.routers.v1 import ROUTERS as v1_routes
from riven.server.rest_api.routers.v1.organizations import router as organizations_router
from riven.server.rest_api.routers.v1.users import router as users_router  # TODO: decide on admin
from riven.server.rest_api.static_files import mount_static_files
from riven.server.rest_api.utils import SENTRY_ENABLED
from riven.server.server import SyncServer
from riven.settings import settings, telemetry_settings
from riven.validators import PATH_VALIDATORS, PRIMITIVE_ID_PATTERNS
...
def generate_openapi_schema(app: FastAPI):
    ...
    riven_docs = app.openapi_schema.copy()
    riven_docs["paths"] = {k: v for k, v in riven_docs["paths"].items() if not k.startswith("/openai")}
    riven_docs["info"]["title"] = "riven API"
    riven_docs["components"]["schemas"]["rivenMessageUnion"] = create_riven_message_union_schema()
    riven_docs["components"]["schemas"]["rivenMessageContentUnion"] = create_riven_message_content_union_schema()
    riven_docs["components"]["schemas"]["rivenAssistantMessageContentUnion"] = create_riven_assistant_message_content_union_schema()
    riven_docs["components"]["schemas"]["rivenToolReturnContentUnion"] = create_riven_tool_return_content_union_schema()
    riven_docs["components"]["schemas"]["rivenUserMessageContentUnion"] = create_riven_user_message_content_union_schema()
    riven_docs["components"]["schemas"]["rivenErrorMessage"] = create_riven_error_message_schema()

    # Update the app's schema with our modified version
    app.openapi_schema = riven_docs
...
random_password = os.getenv("riven_SERVER_PASSWORD") or generate_password()
...
async def lifespan(app_: FastAPI):
    ...
    """
    worker_id = os.getpid()
    from riven.monitoring.readiness_state import initialize_readiness_state, set_readiness_state
    ...
    try:
        ...
        from riven.monitoring.event_loop_watchdog import start_watchdog
    ...
    try:
        ...
        from riven.otel.db_pool_monitoring import setup_pool_monitoring
        from riven.server.db import db_registry, engine as db_engine
    ...
    try:
        from riven.server.rest_api.routers.v1.git_http import set_server_instance
    ...
    try:
        from riven.monitoring.event_loop_watchdog import stop_watchdog
    ...
    try:
        from riven.jobs.scheduler import shutdown_scheduler_and_release_lock
    ...
    if not settings.disable_tracing and settings.sqlalchemy_tracing:
        try:
            from riven.otel.sqlalchemy_instrumentation_integration import teardown_riven_db_instrumentation

            teardown_riven_db_instrumentation()
...
def create_application() -> "FastAPI":
    ...
    # global server
    # server = SyncServer(default_interface_factory=lambda: interface())
    print(f"\n[[ riven server // v{riven_version} ]]")

    if SENTRY_ENABLED:
        sentry_sdk.init(
            dsn=os.getenv("SENTRY_DSN"),
            environment=os.getenv("riven_ENVIRONMENT", "undefined"),
    ...
    if telemetry_settings.enable_datadog:
        try:
            ...
            os.environ.setdefault("DD_VERSION", riven_version)
            ...
            try:
                ...
                if not getattr(OpenAIIntegration, "_riven_provider_patch_done", False):
                    original_set_tags = OpenAIIntegration._llmobs_set_tags

                    def _riven_set_tags(self, span, args, kwargs, response=None, operation=""):
                    ...
                    OpenAIIntegration._llmobs_set_tags = _riven_set_tags
                    OpenAIIntegration._riven_provider_patch_done = True
            except Exception:
    ...
    app.add_exception_handler(rivenInvalidArgumentError, _error_handler_400)
    app.add_exception_handler(rivenToolCreateError, _error_handler_400)
    app.add_exception_handler(rivenToolNameConflictError, _error_handler_400)
    ...
    app.add_exception_handler(rivenImageFetchError, _error_handler_400)
    ...
    app.add_exception_handler(rivenAgentNotFoundError, _error_handler_404_agent)
    app.add_exception_handler(rivenUserNotFoundError, _error_handler_404_user)
    ...
    # 410 Expired errors
    app.add_exception_handler(rivenExpiredError, _error_handler_410)

    # 408 Timeout errors
    app.add_exception_handler(rivenMCPTimeoutError, _error_handler_408)
    app.add_exception_handler(rivenInvalidMCPSchemaError, _error_handler_400)
    ...
    app.add_exception_handler(rivenUnsupportedFileUploadError, _error_handler_415)
    ...
    app.add_exception_handler(rivenServiceUnavailableError, _error_handler_503)
    ...
    @app.exception_handler(rivenMCPConnectionError)
    async def mcp_connection_error_handler(request: Request, exc: rivenMCPConnectionError):
        return JSONResponse(
            status_code=502,
            content={
                "error": {
    ...
    settings.cors_origins.append("https://app.riven.com")

    if (os.getenv("riven_SERVER_SECURE") == "true") or "--secure" in sys.argv:
    ...
    if otlp_endpoint and not settings.disable_tracing:
        ...
        service_name = f"riven-server-{env_name_suffix.lower()}" if env_name_suffix else "riven-server"
        from riven.otel.metrics import setup_metrics
        from riven.otel.tracing import setup_tracing
        ...
        if settings.sqlalchemy_tracing:
            from riven.otel.sqlalchemy_instrumentation_integration import setup_riven_db_instrumentation

            try:
                setup_riven_db_instrumentation(
...
def start_server(
    ...
    if debug:
        from riven.server.server import logger as server_logger
    ...
    if (os.getenv("LOCAL_HTTPS") == "true") or "--localhttps" in sys.argv:
        ...
        print("▶ View using ADE at: https://app.riven.com/development-servers/local/dashboard\n")
        if importlib.util.find_spec("granian") is not None and settings.use_granian:
            from granian import Granian

            # Experimental Granian engine
            Granian(
                target="riven.server.rest_api.app:app",
                # factory=True,
                interface="asgi",
