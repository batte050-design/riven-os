...
from riven.errors import ContextWindowExceededError, RateLimitExceededError
from riven.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns, ns_to_ms
from riven.helpers.message_helper import convert_message_creates_to_messages, resolve_tool_return_images
...
from riven.schemas.riven_message import ToolReturn as rivenToolReturn
...
from riven.schemas.message import ApprovalCreate, Message, MessageCreate, ToolReturn
...
from riven.system import get_heartbeat, package_function_response
    ...
    try:
        ...
        if usage_task is not None:
            try:
                ...
                if not isinstance(usage, rivenUsageStatistics):
                    err_msg = f"Expected rivenUsageStatistics, got {type(usage)}"
...
async def create_approval_response_message_from_input(
    ...
    async def maybe_convert_tool_return_message(maybe_tool_return: rivenToolReturn):
        if isinstance(maybe_tool_return, rivenToolReturn):
...
def create_riven_messages_from_llm_response(
    agent_id: str,
    model: str,
...
def create_assistant_messages_from_openai_response(
    ...
    return create_riven_messages_from_llm_response(
        agent_id=agent_id,
        model=model,
...
def convert_in_context_riven_messages_to_openai(in_context_messages: List[Message], exclude_system_messages: bool = False) -> List[dict]:
    """
    Flattens riven's messages (with system, user, assistant, tool roles, etc.)
