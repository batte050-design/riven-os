...
from riven.agents.riven_agent_batch import rivenAgentBatch
from riven.errors import rivenInvalidArgumentError
from riven.log import get_logger
from riven.schemas.job import BatchJob, JobStatus, JobType, JobUpdate
from riven.schemas.riven_message import rivenMessageSearchResult, rivenMessageUnion, MessageType
from riven.schemas.riven_request import CreateBatch
from riven.schemas.riven_response import rivenBatchMessages
from riven.schemas.message import Message, SearchAllMessagesRequest
...
from riven.validators import MessageId
...
@router.post("/search", response_model=List[rivenMessageSearchResult], operation_id="search_all_messages")
async def search_all_messages(
    ...
    return Message.to_riven_search_results_from_list(search_results=results, text_is_assistant_message=True)
...
async def create_batch(
    ...
    if not settings.enable_batch_job_polling:
        logger.warning("Batch job polling is disabled. Enable batch processing by setting riven_ENABLE_BATCH_JOB_POLLING to True.")
    ...
    try:
        ...
        await batch_runner.step_until_request(batch_requests=payload.requests, riven_batch_job_id=batch_job.id)
...
@router.get(
    ...
    response_model=rivenBatchMessages,
...
async def list_messages_for_batch(
    ...
    messages = await server.batch_manager.get_messages_for_riven_batch_async(
        riven_batch_job_id=batch_id, actor=actor, limit=limit, agent_id=agent_id, sort_descending=(order == "desc"), cursor=after
    )

    return rivenBatchMessages(messages=messages)
...
async def cancel_batch(
    ...
    llm_batch_jobs = await server.batch_manager.list_llm_batch_jobs_async(riven_batch_id=job.id, actor=actor)
...
async def retrieve_message(
    ...
    return message.to_riven_messages()
