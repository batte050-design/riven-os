...
from riven.constants import DEFAULT_GENERATE_TOOL_MODEL_HANDLE
...
from riven.functions.functions import derive_openai_json_schema
from riven.functions.mcp_client.exceptions import MCPTimeoutError
from riven.functions.mcp_client.types import MCPTool, SSEServerConfig, StdioServerConfig, StreamableHTTPServerConfig
from riven.helpers.decorators import deprecated
from riven.llm_api.llm_client import LLMClient
from riven.log import get_logger
from riven.orm.mcp_oauth import OAuthSessionStatus
from riven.prompts.gpt_system import get_system_text
from riven.schemas.enums import AgentType, LLMCallType, MessageRole, ToolType
from riven.schemas.riven_message import ToolReturnMessage
from riven.schemas.riven_message_content import TextContent
from riven.schemas.mcp import UpdateSSEMCPServer, UpdateStdioMCPServer, UpdateStreamableHTTPMCPServer
from riven.schemas.message import Message
from riven.schemas.pip_requirement import PipRequirement
from riven.schemas.tool import Tool, ToolCreate, ToolRunFromSource, ToolSearchRequest, ToolSearchResult, ToolUpdate
from riven.server.rest_api.dependencies import HeaderParams, get_headers, get_riven_server
from riven.server.rest_api.streaming_response import StreamingResponseWithStatusCode
from riven.server.server import SyncServer
from riven.services.mcp.oauth_utils import MCPOAuthSession, drill_down_exception, oauth_stream_event
from riven.services.mcp.stdio_client import AsyncStdioMCPClient
from riven.services.mcp.types import OauthStreamEvent
from riven.settings import tool_settings
from riven.validators import ToolId
...
async def count_tools(
    ...
    return_only_riven_tools: Optional[bool] = Query(False, description="Count only tools with tool_type starting with 'riven_'"),
    exclude_riven_tools: Optional[bool] = Query(False, description="Exclude built-in riven tools from the count"),
    server: SyncServer = Depends(get_riven_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
    ...
    return await server.tool_manager.count_tools_async(
        ...
        exclude_riven_tools=exclude_riven_tools,
...
async def list_tools(
    ...
    return_only_riven_tools: Optional[bool] = Query(False, description="Return only tools with tool_type starting with 'riven_'"),
    server: SyncServer = Depends(get_riven_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
async def list_mcp_tools_by_server(
    ...
    try:
        ...
        raise rivenMCPConnectionError(str(e), server_name=mcp_server_name)
    except HTTPStatusError as e:
        # HTTPStatusError from the MCP server likely means auth issue
        if e.response.status_code == 401:
            raise rivenMCPConnectionError(f"Authentication failed: {e}", server_name=mcp_server_name)
        else:
            raise rivenMCPConnectionError(f"HTTP error from MCP server: {e}", server_name=mcp_server_name)
...
async def add_mcp_tool(
    ...
    """
    Register a new MCP tool as a riven server by MCP server + tool name
    """
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    ...
    if tool_settings.mcp_read_from_config:
        try:
            ...
            raise rivenMCPTimeoutError(str(e), server_name=mcp_server_name)
...
async def add_mcp_server_to_config(
    ...
    """
    Add a new MCP server to the riven MCP server config
    """
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    ...
    if tool_settings.mcp_read_from_config:
        ...
        if isinstance(request, StdioServerConfig) and tool_settings.mcp_disable_stdio:
            raise HTTPException(
                status_code=400,
                detail="stdio is not supported in the current environment, please use a self-hosted riven server in order to add a stdio MCP server",
            )
...
async def test_mcp_server(
    ...
    except (ConnectionError, rivenMCPConnectionError) as e:
        if isinstance(e, rivenMCPConnectionError):
            raise
        raise rivenMCPConnectionError(str(e), server_name=request.server_name)
    except MCPTimeoutError as e:
        raise rivenMCPTimeoutError(f"MCP server connection timed out: {str(e)}", server_name=request.server_name)
    finally:
        if client:
            try:
...
async def generate_json_schema(
    ...
    if request.source_type == "typescript":
        from riven.functions.typescript_parser import derive_typescript_json_schema
    ...
    try:
        ...
        try:
            ...
            if exception_to_check.__class__.__name__ == "ToolError":
                ...
                raise rivenMCPConnectionError(f"MCP tool execution failed: {str(exception_to_check)}", server_name=tool_name)
            raise
