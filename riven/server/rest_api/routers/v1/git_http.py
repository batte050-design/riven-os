...
def set_server_instance(server) -> None:
    """Set the riven server instance for git operations. Called during app startup."""
...
async def _sync_after_push(actor_id: str, agent_id: str) -> None:
    ...
    from riven.services.block_manager_git import GitEnabledBlockManager
    ...
    expected_labels = set()
    from riven.services.memory_repo.block_markdown import parse_block_markdown
...
async def proxy_git_http(
    path: str,
    request: Request,
    server=Depends(get_riven_server),
    headers: HeaderParams = Depends(get_headers),
):
    """Proxy `/v1/git/*` requests to the memfs service.

    Requires riven_MEMFS_SERVICE_URL to be configured.
    """
    ...
    if not memfs_url:
        return JSONResponse(
            status_code=501,
            content={
                "detail": "git HTTP requires memfs service (riven_MEMFS_SERVICE_URL not configured)",
            },
        )
