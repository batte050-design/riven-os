...
from riven.schemas.group import Group, GroupCreate, GroupUpdate, ManagerType
...
from riven.validators import GroupId, MessageId
...
@router.patch("/{group_id}/messages/{message_id}", response_model=rivenMessageUnion, operation_id="modify_group_message", deprecated=True)
async def modify_group_message(
    ...
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    return await server.message_manager.update_message_by_riven_message(message_id=message_id, riven_message_update=request, actor=actor)
