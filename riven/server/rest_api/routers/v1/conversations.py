...
from riven.data_sources.redis_client import NoopAsyncRedisClient, get_redis_client
from riven.errors import ConversationBusyError, rivenExpiredError, rivenInvalidArgumentError, NoActiveRunsToCancelError
from riven.helpers.datetime_helpers import get_utc_time
from riven.log import get_logger
from riven.schemas.conversation import Conversation, CreateConversation, UpdateConversation
from riven.schemas.enums import RunStatus
from riven.schemas.job import rivenRequestConfig
from riven.schemas.riven_message import rivenMessageUnion, MessageType
from riven.schemas.riven_request import ConversationMessageRequest, rivenStreamingRequest, RetrieveStreamRequest
from riven.schemas.riven_response import rivenResponse
from riven.schemas.provider_trace import BillingContext
from riven.schemas.run import Run as PydanticRun
from riven.server.rest_api.dependencies import HeaderParams, get_headers, get_riven_server
from riven.server.rest_api.redis_stream_manager import redis_sse_stream_generator
...
from riven.validators import ConversationId, ConversationIdOrDefault
...
async def list_conversation_messages(
    ...
    Returns rivenMessage objects (UserMessage, AssistantMessage, etc.) for all
...
async def _send_agent_direct_message(
    ...
    if request.streaming:
        streaming_request = rivenStreamingRequest(
    ...
    from riven.services.streaming_service import derive_request_token, enrich_conversation_busy_error
...
async def send_conversation_message(
    ...
    **Deprecated**: Passing an agent ID as conversation_id still works but will be removed.
    ...
    # Streaming mode (default)
    if request.streaming:
        # Convert to rivenStreamingRequest for StreamingService compatibility
        streaming_request = rivenStreamingRequest(
            messages=request.messages,
            streaming=True,
            stream_tokens=request.stream_tokens,
                ...
                "text/event-stream": {
                    ...
                    "schema": {
                        "oneOf": [
                            ...
                            {"$ref": "#/components/schemas/rivenPing"},
                            {"$ref": "#/components/schemas/rivenErrorMessage"},
                            {"$ref": "#/components/schemas/rivenStopReason"},
                            {"$ref": "#/components/schemas/rivenUsageStatistics"},
                        ]
                    },
                },
            },
...
async def retrieve_conversation_stream(
    ...
    **Deprecated**: Passing an agent ID as conversation_id still works but will be removed.
    ...
    # Priority 3: Fall back to active run lookup
        ...
        if not active_runs:
            raise rivenInvalidArgumentError("No active runs found for this conversation.")
        ...
        if not run.background:
            raise rivenInvalidArgumentError("Run was not created in background mode, so it cannot be retrieved.")

    # Check expiration for all paths
    if run and run.created_at < get_utc_time() - timedelta(hours=3):
        raise rivenExpiredError("Run was created more than 3 hours ago, and is now expired.")
    ...
    if settings.enable_cancellation_aware_streaming:
        from riven.server.rest_api.streaming_response import cancellation_aware_stream_wrapper, get_cancellation_event_for_run
...
async def recompile_conversation(
    ...
    if not dry_run:
        ...
        from riven.schemas.message import MessageUpdate
...
async def compact_conversation(
    ...
    **Deprecated**: Passing an agent ID as conversation_id still works but will be removed.
    ...
    if resolved_agent_id:
        ...
        agent_loop = rivenAgentV3(agent_state=agent, actor=actor, conversation_id=conversation_id)
