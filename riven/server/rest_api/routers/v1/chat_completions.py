...
from riven.schemas.riven_request import rivenStreamingRequest
...
async def _handle_chat_completion(
    ...
    # convert OpenAI messages to riven MessageCreate format
    # NOTE: we only process the last user message
    if len(request.messages) > 1:
        logger.warning(
            f"Chat completions endpoint received {len(request.messages)} messages. "
            "riven maintains conversation state internally, so only the last user message will be processed. "
    ...
    riven_request = rivenStreamingRequest(
        messages=riven_messages,
        stream_tokens=True,
    )
...
async def create_chat_completion(
    ...
    """
    Create a chat completion using a riven agent (OpenAI-compatible).
