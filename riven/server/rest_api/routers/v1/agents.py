...
from riven.agents.agent_loop import AgentLoop
from riven.agents.base_agent_v2 import BaseAgentV2
from riven.agents.riven_agent import rivenAgent
from riven.agents.riven_agent_v3 import rivenAgentV3
from riven.constants import DEFAULT_MAX_STEPS, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, REDIS_RUN_ID_PREFIX
from riven.data_sources.redis_client import get_redis_client
...
from riven.schemas.agent import AgentRelationships, AgentState, CreateAgent, UpdateAgent
from riven.schemas.agent_file import AgentFileSchema, SkillSchema
from riven.schemas.block import BlockResponse, BlockUpdate
from riven.schemas.enums import AgentType, MessageRole, RunStatus
from riven.schemas.file import AgentFileAttachment, PaginatedAgentFiles
from riven.schemas.group import Group
from riven.schemas.job import rivenRequestConfig
from riven.schemas.riven_message import rivenMessageUnion, rivenMessageUpdateUnion, MessageType
from riven.schemas.riven_message_content import TextContent
from riven.schemas.riven_request import rivenAsyncRequest, rivenRequest, rivenStreamingRequest
from riven.schemas.riven_response import rivenResponse, rivenStreamingResponse
from riven.schemas.riven_stop_reason import StopReasonType
from riven.schemas.mcp_server import ToolExecuteRequest
...
from riven.schemas.message import Message, MessageCreate, MessageCreateType, MessageSearchRequest, MessageSearchResult
from riven.schemas.passage import Passage
from riven.schemas.provider_trace import BillingContext
from riven.schemas.run import Run as PydanticRun, RunUpdate
...
from riven.serialize_schemas.pydantic_agent_schema import AgentSchema
from riven.server.rest_api.dependencies import HeaderParams, get_headers, get_riven_server
from riven.server.server import SyncServer
from riven.services.lettuce import LettuceClient
from riven.services.run_manager import RunManager
from riven.services.streaming_service import StreamingService
from riven.services.summarizer.summarizer_config import CompactionSettings
from riven.settings import settings
from riven.utils import is_1_0_sdk_version, safe_create_shielded_task, safe_create_task, truncate_file_visible_content
from riven.validators import AgentId, BlockId, FileId, MessageId, SourceId, ToolId
...
class GenerateResponse(BaseModel):
    ...
    usage: rivenUsageStatistics = Field(..., description="Token usage statistics")
...
async def list_agents(
    ...
    server: SyncServer = Depends(get_riven_server),
...
async def _import_agent(
    ...
    if not import_result.success:
        from riven.errors import AgentFileImportError
...
async def import_agent(
    ...
    override_existing_tools: bool = Form(
        True,
        description="If set to True, existing tools can get their source code overwritten by the uploaded tool definitions. Note that riven core tools can never be updated externally.",
    ),
...
async def open_file_for_agent(
    ...
    from riven.services.file_processor.chunker.line_chunker import LineChunker
...
AgentMessagesResponse = Annotated[
    list[rivenMessageUnion], Field(json_schema_extra={"type": "array", "items": {"$ref": "#/components/schemas/rivenMessageUnion"}})
]
...
@router.patch("/{agent_id}/messages/{message_id}", response_model=rivenMessageUnion, operation_id="modify_message", deprecated=True)
async def modify_message(
    agent_id: AgentId,  # backwards compatible. Consider removing for v1
    message_id: MessageId,
    request: rivenMessageUpdateUnion = Body(...),
    server: "SyncServer" = Depends(get_riven_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
@router.post(
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {"schema": {"$ref": "#/components/schemas/rivenResponse"}},
...
async def send_message(
    ...
    agent_id: AgentId,
    server: SyncServer = Depends(get_riven_server),
    request: rivenStreamingRequest = Body(...),
    headers: HeaderParams = Depends(get_headers),
) -> StreamingResponse | rivenResponse:
    """
    ...
    **Note:** Sending multiple concurrent requests to the same agent can lead to undefined behavior.
    ...
    - If `streaming=false` (default): Returns a complete rivenResponse with all messages
    ...
    if settings.track_agent_run:
        ...
        run = await runs_manager.create_run(
            pydantic_run=PydanticRun(
                ...
                request_config=rivenRequestConfig.from_riven_request(request),
            ),
            actor=actor,
        )
    else:
...
@router.post(
    ...
    response_model=rivenStreamingResponse,
...
async def _process_message_background(
    ...
    try:
        ...
        runs_manager = RunManager()
        from riven.schemas.enums import RunStatus
        from riven.schemas.riven_stop_reason import StopReasonType
...
async def _cleanup_background_task_resources(agent_loop: BaseAgentV2 | rivenAgent, result: StreamingResponse | rivenResponse) -> None:
    """
...
async def send_message_async(
    agent_id: AgentId,
    server: SyncServer = Depends(get_riven_server),
    request: rivenAsyncRequest = Body(...),
    headers: HeaderParams = Depends(get_headers),
):
    """
    ...
    **Note:** Sending multiple concurrent requests to the same agent can lead to undefined behavior.
    ...
    if use_lettuce:
        ...
        is_v1_message_async_enabled = (
            agent_state.agent_type == AgentType.riven_v1_agent and headers.experimental_params.riven_v1_agent_message_async
        )
        if agent_state.multi_agent_group is None and (agent_state.agent_type != AgentType.riven_v1_agent or is_v1_message_async_enabled):
            lettuce_client = await LettuceClient.create()
...
async def preview_model_request(
    agent_id: AgentId,
    request: Union[rivenRequest, rivenStreamingRequest] = Body(...),
    server: SyncServer = Depends(get_riven_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
async def summarize_messages(
    ...
    agent_loop = rivenAgentV3(agent_state=agent, actor=actor)
    ...
    if agent.compaction_settings and request and request.compaction_settings:
        ...
        if (
            ...
            from riven.services.summarizer.summarizer_config import get_default_prompt_for_mode
