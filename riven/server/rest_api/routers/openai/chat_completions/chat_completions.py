...
from riven.agent import Agent
from riven.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, riven_MODEL_ENDPOINT
...
from riven.server.rest_api.chat_completions_interface import ChatCompletionsStreamingInterface
from riven.server.rest_api.dependencies import HeaderParams, get_headers, get_riven_server

# TODO this belongs in a controller!
from riven.server.rest_api.utils import get_user_message_from_chat_completions_request, sse_async_generator
...
async def create_chat_completions(
    ...
    riven_agent = server.load_agent(agent_id=agent_id, actor=actor)
    llm_config = riven_agent.agent_state.llm_config
    if llm_config.model_endpoint_type != "openai" or llm_config.model_endpoint == riven_MODEL_ENDPOINT:
...
async def send_message_to_agent_chat_completions(
    ...
    try:
        ...
        riven_agent.interface = ChatCompletionsStreamingInterface()
        streaming_interface = riven_agent.interface
        ...
        safe_create_task(
            asyncio.to_thread(
                ...
                actor=actor,
                agent_id=riven_agent.agent_state.id,
