...
import riven.constants as constants
import riven.server.utils as server_utils
from riven.config import rivenConfig
from riven.constants import riven_TOOL_EXECUTION_DIR
from riven.data_sources.connectors import DataConnector, load_data
from riven.errors import (
    HandleNotFoundError,
    rivenInvalidArgumentError,
    rivenMCPConnectionError,
)
from riven.functions.mcp_client.types import MCPServerType, MCPTool, MCPToolHealth, SSEServerConfig, StdioServerConfig
from riven.functions.schema_validator import validate_complete_json_schema
from riven.helpers.datetime_helpers import get_utc_time

# TODO use custom interface
from riven.interface import (
    CLIInterface,  # for printing to terminal
)
from riven.log import get_logger
from riven.orm.errors import NoResultFound
from riven.otel.tracing import log_event, trace_method
from riven.prompts.gpt_system import get_system_text
from riven.schemas.agent import AgentState, CreateAgent, UpdateAgent
from riven.schemas.block import Block, BlockUpdate, CreateBlock
from riven.schemas.embedding_config import EmbeddingConfig

# openai schemas
from riven.schemas.enums import AgentType, JobStatus, ProviderCategory, ProviderType, ToolSourceType
from riven.schemas.group import GroupCreate, SleeptimeManager, VoiceSleeptimeManager
from riven.schemas.job import Job, JobUpdate
from riven.schemas.riven_message import rivenMessage, MessageType, ToolReturnMessage
from riven.schemas.llm_config import LLMConfig
from riven.schemas.memory import Memory
from riven.schemas.message import Message
from riven.schemas.passage import Passage
from riven.schemas.pip_requirement import PipRequirement
from riven.schemas.providers import (
    AnthropicProvider,
    AzureProvider,
    BasetenProvider,
    BedrockProvider,
...
from riven.schemas.sandbox_config import LocalSandboxConfig, SandboxConfigCreate
from riven.schemas.secret import Secret
from riven.schemas.source import Source
from riven.schemas.tool import Tool
from riven.schemas.user import User
from riven.services.agent_manager import AgentManager
from riven.services.agent_serialization_manager import AgentSerializationManager
from riven.services.archive_manager import ArchiveManager
from riven.services.block_manager import BlockManager
from riven.services.block_manager_git import GIT_MEMORY_ENABLED_TAG, GitEnabledBlockManager
from riven.services.file_manager import FileManager
from riven.services.files_agents_manager import FileAgentManager
from riven.services.group_manager import GroupManager
from riven.services.helpers.tool_execution_helper import prepare_local_sandbox
from riven.services.identity_manager import IdentityManager
from riven.services.job_manager import JobManager
from riven.services.llm_batch_manager import LLMBatchManager
from riven.services.mcp.base_client import AsyncBaseMCPClient
from riven.services.mcp.fastmcp_client import AsyncFastMCPSSEClient
from riven.services.mcp.sse_client import MCP_CONFIG_TOPLEVEL_KEY
from riven.services.mcp.stdio_client import AsyncStdioMCPClient
from riven.services.mcp_manager import MCPManager
from riven.services.mcp_server_manager import MCPServerManager
from riven.services.memory_repo import MemfsClient
from riven.services.message_manager import MessageManager
from riven.services.organization_manager import OrganizationManager
from riven.services.passage_manager import PassageManager
from riven.services.provider_manager import ProviderManager
from riven.services.run_manager import RunManager
from riven.services.sandbox_config_manager import SandboxConfigManager
from riven.services.source_manager import SourceManager
from riven.services.step_manager import StepManager
from riven.services.telemetry_manager import TelemetryManager
from riven.services.tool_executor.tool_execution_manager import ToolExecutionManager
from riven.services.tool_manager import ToolManager
from riven.services.user_manager import UserManager
from riven.settings import DatabaseChoice, model_settings, settings, tool_settings
from riven.streaming_interface import AgentChunkStreamingInterface
from riven.utils import get_friendly_error_msg, get_persona_text

config = rivenConfig.load()
...
class SyncServer(object):
    ...
    def __init__(
        ...
        if settings.database_engine is DatabaseChoice.POSTGRES:
            ...
            config.recall_storage_uri = settings.riven_pg_uri_no_default
            config.archival_storage_type = "postgres"
            config.archival_storage_uri = settings.riven_pg_uri_no_default
        config.save()
        self.config = config
        ...
        from riven.services.agent_generate_completion_manager import AgentGenerateCompletionManager
        ...
        # collect providers (always has riven as a default)
        from riven.constants import riven_MODEL_ENDPOINT

        self._enabled_providers: List[Provider] = [rivenProvider(name="riven", base_url=riven_MODEL_ENDPOINT)]
    ...
    async def init_async(self, init_with_default_org_and_user: bool = True):
        ...
        if init_with_default_org_and_user:
            ...
            tool_dir = tool_settings.tool_exec_dir or riven_TOOL_EXECUTION_DIR

            venv_dir = Path(tool_dir) / venv_name
            tool_path = Path(tool_dir)

            if tool_path.exists() and not tool_path.is_dir():
                logger.error(f"riven_TOOL_SANDBOX_DIR exists but is not a directory: {tool_dir}")
            else:
                if not tool_path.exists():
                    logger.warning(f"riven_TOOL_SANDBOX_DIR does not exist, creating now: {tool_dir}")
                    tool_path.mkdir(parents=True, exist_ok=True)

                if tool_settings.tool_exec_venv_name and not venv_dir.is_dir():
                    logger.warning(
                        f"Provided riven_TOOL_SANDBOX_VENV_NAME is not a valid venv ({venv_dir}), one will be created for you during tool execution."
                    )
    ...
    def _init_memory_repo_manager(self) -> Optional[MemfsClient]:
        ...
        Requires riven_MEMFS_SERVICE_URL to be set to the external memfs service URL.

        Returns:
            MemfsClient if configured, None otherwise
        """
        from riven.settings import settings
    ...
    async def init_mcp_clients(self):
        ...
        for server_name, server_config in mcp_server_configs.items():
            if server_config.type == MCPServerType.SSE:
                ...
                raise rivenInvalidArgumentError(f"Invalid MCP server config: {server_config}", argument_name="server_config")
        ...
        if request.llm_config is None:
            ...
            if request.model is None:
                if settings.default_llm_handle is None:
                    raise rivenInvalidArgumentError("Must specify either model or llm_config in request", argument_name="model")
                else:
                ...
                if isinstance(request.model, str):
                    ...
                    raise rivenInvalidArgumentError("Multiple models are not supported yet")
                else:
    ...
    async def insert_archival_memory_async(
        ...
        from riven.services.context_window_calculator.token_counter import create_token_counter
        from riven.settings import settings
    ...
    async def get_agent_recall(
    ...
    ) -> Union[List[Message], List[rivenMessage]]:
        ...
        if not return_message_object:
            records = Message.to_riven_messages_from_list(
                messages=records,
    ...
    async def get_agent_recall_async(
        ...
        if not return_message_object:
            ...
            agent_state = await self.agent_manager.get_agent_by_id_async(agent_id=agent_id, actor=actor)
            text_is_assistant_message = agent_state.agent_type == AgentType.riven_v1_agent
    ...
    async def get_all_messages_recall_async(
        ...
        if not return_message_object:
            # NOTE: We are assuming all messages are coming from riven_v1_agent. This may lead to slightly incorrect assistant message handling.
            # text_is_assistant_message = agent_state.agent_type == AgentType.riven_v1_agent
    ...
    def get_server_config(self, include_defaults: bool = False) -> dict:
        ...
        if include_defaults:
            default_config = vars(rivenConfig())
    ...
    async def load_file_to_source(self, source_id: str, file_path: str, job_id: str, actor: User) -> Job:
        ...
        # try:
        from riven.data_sources.connectors import DirectoryConnector
    ...
    async def list_llm_models_async(
        ...
        if include_base:
            ...
            from riven.services.provider_manager import AUTO_MODE_HANDLES
    ...
    async def get_embedding_config_from_handle_async(
        ...
        try:
            ...
            # Convert to rivenInvalidArgumentError for backwards compatibility
            from riven.orm.errors import NoResultFound

            if isinstance(e, NoResultFound):
                raise rivenInvalidArgumentError(f"Embedding model {handle} not found", argument_name="handle")
            raise
    ...
    async def run_tool_from_source(
        ...
        from riven.services.tool_schema_generator import generate_schema_for_tool_creation
        ...
        try:
            ...
            from riven.schemas.riven_message import ToolReturn as ToolReturnSchema
    ...
    async def get_mcp_servers(self) -> dict[str, Union[SSEServerConfig, StdioServerConfig]]:
        ...
        # Attempt to read from ~/.riven/mcp_config.json
        mcp_config_path = os.path.join(constants.riven_DIR, constants.MCP_CONFIG_NAME)
    ...
    async def get_tools_from_mcp_server(self, mcp_server_name: str) -> List[MCPTool]:
        ...
        if mcp_server_name not in self.mcp_clients:
            raise rivenInvalidArgumentError(f"No client was created for MCP server: {mcp_server_name}", argument_name="mcp_server_name")
    ...
    async def add_mcp_server_to_config(
        ...
        try:
            ...
            raise rivenInvalidArgumentError(f"Failed to parse MCP config file {mcp_config_path}")
        ...
        try:
            ...
            raise rivenInvalidArgumentError(f"Failed to write MCP config file {mcp_config_path}")
    ...
    async def delete_mcp_server_from_config(self, server_name: str) -> dict[str, Union[SSEServerConfig, StdioServerConfig]]:
        ...
        if server_name not in current_mcp_servers:
            raise rivenInvalidArgumentError(f"Server name {server_name} not found in MCP config file", argument_name="server_name")
