...
from riven.functions.mcp_client.types import StdioServerConfig
from riven.helpers.json_helpers import sanitize_null_bytes
from riven.log import get_logger

logger = get_logger(__name__)
from riven.schemas.embedding_config import EmbeddingConfig
from riven.schemas.enums import ProviderType, ToolRuleType
from riven.schemas.riven_message import ApprovalReturn, MessageReturnType
...
from riven.schemas.llm_batch_job import AgentStepState
from riven.schemas.llm_config import LLMConfig
from riven.schemas.message import ToolReturn
from riven.schemas.response_format import (
...
from riven.schemas.tool_rule import (
    ChildToolRule,
    ConditionalToolRule,
    ContinueToolRule,
    InitToolRule,
    ...
    for content in message_content:
        if isinstance(content, MessageContent):
            if content.type == MessageContentType.image:
                assert content.source.type == ImageSourceType.riven, f"Invalid image source type: {content.source.type}"
...
def deserialize_message_content(data: Optional[List[Dict]]) -> List[MessageContent]:
    ...
    for item in data:
        ...
        if content_type == MessageContentType.text:
            ...
            assert item["source"]["type"] == ImageSourceType.riven, f"Invalid image source type: {item['source']['type']}"
