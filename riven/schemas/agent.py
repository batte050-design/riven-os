...
from riven.errors import AgentExportProcessingError, rivenInvalidArgumentError
from riven.schemas.block import Block, CreateBlock
from riven.schemas.embedding_config import EmbeddingConfig
from riven.schemas.enums import PrimitiveType
from riven.schemas.environment_variables import AgentEnvironmentVariable
from riven.schemas.group import Group
from riven.schemas.identity import Identity
from riven.schemas.riven_base import OrmMetadataBase
from riven.schemas.riven_message import ApprovalRequestMessage
...
from riven.schemas.model import ModelSettingsUnion
from riven.schemas.openai.chat_completion_response import UsageStatistics
from riven.schemas.response_format import ResponseFormatUnion
from riven.schemas.source import Source
from riven.schemas.tool import Tool
from riven.schemas.tool_rule import ToolRule
from riven.services.summarizer.summarizer_config import CompactionSettings
from riven.utils import calculate_file_defaults_based_on_context_window, create_random_username
from riven.validators import BlockId, IdentityId, MessageId, SourceId, ToolId
...
class AgentType(str, Enum):
    ...
    riven_v1_agent = "riven_v1_agent"  # simplification of the memgpt loop, no heartbeats or forced tool calls
...
class CreateAgent(BaseModel, validate_assignment=True):  #
    ...
    agent_type: AgentType = Field(default_factory=lambda: AgentType.riven_v1_agent, description="The type of agent.")
    ...
    include_base_tools: bool = Field(True, description="If true, attaches the riven core tools (e.g. core_memory related functions).")
    include_multi_agent_tools: bool = Field(
        False, description="If true, attaches the riven multi-agent tools (e.g. sending a message to another agent)."
    )
    include_base_tool_rules: Optional[bool] = Field(
        None, description="If true, attaches the riven base tool rules (e.g. deny all tools not explicitly allowed)."
    )
    ...
    def validate_model(cls, model: Optional[str]) -> Optional[str]:
        ...
        if "/" not in model:
            raise rivenInvalidArgumentError("The model handle should be in the format provider/model-name", argument_name="model")
