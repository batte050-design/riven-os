...
from riven.schemas.riven_stop_reason import rivenStopReason
from riven.schemas.message import Message
from riven.schemas.openai.chat_completion_response import ChoiceLogprobs
...
class rivenResponse(BaseModel):
    ...
    The type of the returned messages can be either `Message` or `rivenMessage`, depending on what was specified in the request.

    Attributes:
        messages (List[Union[Message, rivenMessage]]): The messages returned by the agent.
        usage (rivenUsageStatistics): The usage statistics
    """

    messages: List[rivenMessageUnion] = Field(
        ...,
        description="The messages returned by the agent.",
        json_schema_extra={
            "items": {
                "$ref": "#/components/schemas/rivenMessageUnion",
            }
        },
    )
    stop_reason: rivenStopReason = Field(
        ...,
        description="The stop reason from riven indicating why agent loop stopped execution.",
    )
    usage: rivenUsageStatistics = Field(
        ...,
        ...
        return json_dumps(
            {
                ...
                # Assume `Message` and `rivenMessage` have a `dict()` method
                "usage": self.usage.model_dump(),  # Assume `rivenUsageStatistics` has a `dict()` method
            },
            indent=4,
        )
...
class rivenStreamingResponse(RootModel):
    """
...
class rivenBatchResponse(BaseModel):
    riven_batch_id: str = Field(..., description="A unique identifier for the riven batch request.")
...
class rivenBatchMessages(BaseModel):
