...
from riven.schemas.riven_message_content import (
    rivenAssistantMessageContentUnion,
    rivenToolReturnContentUnion,
    rivenUserMessageContentUnion,
    get_riven_assistant_message_content_union_str_json_schema,
    get_riven_tool_return_content_union_str_json_schema,
    get_riven_user_message_content_union_str_json_schema,
)

# ---------------------------
# riven API Messaging Schemas
# ---------------------------
...
class ToolReturn(MessageReturn):
    ...
    tool_return: Union[str, List[rivenToolReturnContentUnion]] = Field(
        ...,
        description="The tool return value - either a string or list of content parts (text/image)",
        json_schema_extra=get_riven_tool_return_content_union_str_json_schema(),
    )
...
rivenMessageReturnUnion = Annotated[Union[ApprovalReturn, ToolReturn], Field(discriminator="type")]
...
class rivenMessage(BaseModel):
    """
    Base class for simplified riven message response type. This is intended to be used for developers
...
class SystemMessage(rivenMessage):
    """
...
class UserMessage(rivenMessage):
    """
    ...
    Args:
        ...
        date (datetime): The date the message was created in ISO format
        name (Optional[str]): The name of the sender of the message
        content (Union[str, List[rivenUserMessageContentUnion]]): The message content sent by the user (can be a string or an array of multi-modal content parts)
    """

    message_type: Literal[MessageType.user_message] = Field(default=MessageType.user_message, description="The type of the message.")
    content: Union[str, List[rivenUserMessageContentUnion]] = Field(
        ...,
        description="The message content sent by the user (can be a string or an array of multi-modal content parts)",
        json_schema_extra=get_riven_user_message_content_union_str_json_schema(),
    )


class ReasoningMessage(rivenMessage):
    """
...
class HiddenReasoningMessage(rivenMessage):
    """
...
class ToolCallMessage(rivenMessage):
    """
...
class ToolReturnMessage(rivenMessage):
    """
    A message representing the return value of a tool call (generated by riven executing the requested tool).
...
class ApprovalRequestMessage(rivenMessage):
    """
...
class ApprovalResponseMessage(rivenMessage):
    """
...
class AssistantMessage(rivenMessage):
    """
    ...
    Args:
        ...
        date (datetime): The date the message was created in ISO format
        name (Optional[str]): The name of the sender of the message
        content (Union[str, List[rivenAssistantMessageContentUnion]]): The message content sent by the agent (can be a string or an array of content parts)
    """
    ...
    content: Union[str, List[rivenAssistantMessageContentUnion]] = Field(
        ...,
        description="The message content sent by the agent (can be a string or an array of content parts)",
        json_schema_extra=get_riven_assistant_message_content_union_str_json_schema(),
    )


class rivenPing(rivenMessage):
    """
...
class rivenErrorMessage(BaseModel):
    """
...
class SummaryMessage(rivenMessage):
    """
...
class EventMessage(rivenMessage):
    """
...
def create_riven_message_union_schema():
    return {
        "oneOf": [
...
def create_riven_error_message_schema():
    return {
        "properties": {
            "message_type": {
                "type": "string",
...
rivenMessageUpdateUnion = Annotated[
...
rivenMessageSearchResult = Annotated[
    Union[
...
class LegacyFunctionCallMessage(rivenMessage):
    function_call: str


class LegacyFunctionReturn(rivenMessage):
    """
    A message representing the return value of a function call (generated by riven executing the requested function).
...
class LegacyInternalMonologue(rivenMessage):
    """
...
LegacyrivenMessage = Union[LegacyInternalMonologue, AssistantMessage, LegacyFunctionCallMessage, LegacyFunctionReturn]
