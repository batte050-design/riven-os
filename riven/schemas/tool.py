...
from riven.constants import (
    ...
    riven_BUILTIN_TOOL_MODULE_NAME,
    riven_CORE_TOOL_MODULE_NAME,
    riven_FILES_TOOL_MODULE_NAME,
    riven_MULTI_AGENT_TOOL_MODULE_NAME,
    riven_VOICE_TOOL_MODULE_NAME,
...
from riven.functions.functions import get_json_schema_from_module
from riven.functions.mcp_client.types import MCPTool
from riven.functions.schema_generator import generate_tool_schema_for_mcp
from riven.log import get_logger
from riven.schemas.enums import ToolType
from riven.schemas.riven_base import rivenBase
from riven.schemas.npm_requirement import NpmRequirement
from riven.schemas.pip_requirement import PipRequirement

logger = get_logger(__name__)


class BaseTool(rivenBase):
...
class Tool(BaseTool):
    ...
    def refresh_source_code_and_json_schema(self):
        ...
        This method only handles built-in riven tools.
        """
        ...
        elif self.tool_type in {ToolType.riven_CORE, ToolType.riven_MEMORY_CORE, ToolType.riven_SLEEPTIME_CORE}:
            # If it's riven core tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=riven_CORE_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.riven_MULTI_AGENT_CORE}:
            # If it's riven multi-agent tool, we also generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=riven_MULTI_AGENT_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.riven_VOICE_SLEEPTIME_CORE}:
            # If it's riven voice tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=riven_VOICE_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.riven_BUILTIN}:
            # If it's riven voice tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=riven_BUILTIN_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.riven_FILES_CORE}:
            # If it's riven files tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=riven_FILES_TOOL_MODULE_NAME, function_name=self.name)

        return self


class ToolCreate(rivenBase):
    description: Optional[str] = Field(None, description="The description of the tool.")
    ...
    def from_mcp(cls, mcp_server_name: str, mcp_tool: MCPTool) -> "ToolCreate":
        from riven.functions.helpers import generate_mcp_tool_wrapper
    ...
    def model_dump(self, to_orm: bool = False, **kwargs):
        """
        Override rivenBase.model_dump to explicitly handle 'tags' being None,
...
class ToolRunFromSource(rivenBase):
    source_code: str = Field(..., description="The source code of the function.")
...
class ToolSearchRequest(rivenBase):
    ...
    tool_types: Optional[List[str]] = Field(None, description="Filter by tool types (e.g., 'custom', 'riven_core').")
...
class ToolSearchResult(rivenBase):
