...
class BaseMCPServer(rivenBase):
    __id_prefix__ = PrimitiveType.MCP_SERVER.value


# Create Schemas (for POST requests)
class CreateStdioMCPServer(rivenBase):
...
class CreateSSEMCPServer(rivenBase):
...
class CreateStreamableHTTPMCPServer(rivenBase):
...
class UpdateStdioMCPServer(rivenBase):
...
class UpdateSSEMCPServer(rivenBase):
...
class UpdateStreamableHTTPMCPServer(rivenBase):
...
class MCPServerResyncResult(rivenBase):
...
class ToolExecuteRequest(rivenBase):
...
class CreateMCPServerRequest(rivenBase):
...
class UpdateMCPServerRequest(rivenBase):
...
def convert_update_to_internal(request: UpdateMCPServerRequest):
    ...
    from riven.functions.mcp_client.types import MCPServerType as MCPType, StdioServerConfig as StdioCfg
