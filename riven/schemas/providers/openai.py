...
from riven.constants import DEFAULT_EMBEDDING_CHUNK_SIZE, LLM_MAX_CONTEXT_WINDOW
from riven.errors import ErrorCode, LLMAuthenticationError, LLMError, LLMPermissionDeniedError
...
class OpenAIProvider(Provider):
    ...
    async def get_default_max_output_tokens_async(self, model_name: str) -> int:
        ...
        """
        from riven.model_specs.litellm_model_specs import get_max_output_tokens
    ...
    async def _list_llm_models(self, data: list[dict]) -> list[LLMConfig]:
        """
        This handles filtering out LLM Models by provider that meet riven's requirements.
        """
        configs = []
        for model in data:
    ...
    def _set_model_parameter_tuned_defaults(model_name: str, llm_config: LLMConfig):
        ...
        if "gpt-4o" in model_name or "gpt-4.1-mini" in model_name or model_name == "riven-free":
    ...
    async def get_model_context_window_size_async(self, model_name: str) -> int | None:
        ...
        """
        from riven.model_specs.litellm_model_specs import get_context_window
