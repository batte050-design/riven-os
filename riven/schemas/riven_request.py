...
from riven.constants import DEFAULT_MAX_STEPS, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from riven.schemas.riven_message import MessageType
from riven.schemas.riven_message_content import rivenMessageContentUnion
from riven.schemas.message import MessageCreate, MessageCreateUnion, MessageRole
from riven.validators import AgentId
...
class rivenRequest(BaseModel):
    ...
    use_assistant_message: bool = Field(
        default=True,
        description="Whether the server should parse specific tool call arguments (default `send_message`) as `AssistantMessage` objects. Still supported for legacy agent types, but deprecated for riven_v1_agent onward.",
        deprecated=True,
    )
    assistant_message_tool_name: str = Field(
        default=DEFAULT_MESSAGE_TOOL,
        description="The name of the designated message tool. Still supported for legacy agent types, but deprecated for riven_v1_agent onward.",
        deprecated=True,
    )
    assistant_message_tool_kwarg: str = Field(
        default=DEFAULT_MESSAGE_TOOL_KWARG,
        description="The name of the message argument in the designated message tool. Still supported for legacy agent types, but deprecated for riven_v1_agent onward.",
        deprecated=True,
    )
    ...
    def validate_input_or_messages(self):
        ...
        # input can be either a string or List[rivenMessageContentUnion]
        if self.input is not None:
            # Both str and List[rivenMessageContentUnion] are valid content types for MessageCreate
            self.messages = [MessageCreate(role=MessageRole.user, content=self.input)]

        return self


class rivenStreamingRequest(rivenRequest):
...
class ConversationMessageRequest(rivenRequest):
