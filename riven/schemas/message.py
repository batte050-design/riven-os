...
from riven.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, REQUEST_HEARTBEAT_PARAM, TOOL_CALL_ID_MAX_LEN
from riven.helpers.datetime_helpers import get_utc_time, is_utc_datetime
from riven.helpers.json_helpers import json_dumps
from riven.local_llm.constants import INNER_THOUGHTS_KWARG, INNER_THOUGHTS_KWARG_VERTEX
from riven.otel.tracing import trace_method
from riven.schemas.enums import MessageRole, PrimitiveType
from riven.schemas.riven_base import OrmMetadataBase
from riven.schemas.riven_message import (
    ...
    rivenMessage,
    rivenMessageReturnUnion,
    rivenMessageSearchResult,
    MessageType,
    ReasoningMessage,
...
from riven.schemas.riven_message_content import (
    ImageContent,
    ImageSourceType,
    rivenMessageContentUnion,
    rivenToolReturnContentUnion,
    ...
    ToolCallContent,
    ToolReturnContent,
    get_riven_message_content_union_str_json_schema,
)
from riven.system import unpack_message
from riven.utils import parse_json, parse_json_or_wrap_raw, sanitize_tool_call_id, validate_function_response
...
class MessageCreate(MessageCreateBase):
    ...
    content: Union[str, List[rivenMessageContentUnion]] = Field(
        ...,
        description="The content of the message.",
        json_schema_extra=get_riven_message_content_union_str_json_schema(),
    )
...
class ApprovalCreate(MessageCreateBase):
    ...
    approvals: Optional[List[rivenMessageReturnUnion]] = Field(default=None, description="The list of approval responses")
...
class ToolReturnCreate(MessageCreateBase):
    ...
    tool_returns: List[rivenToolReturn] = Field(
        ...,
...
class MessageUpdate(BaseModel):
    ...
    content: Optional[Union[str, List[rivenMessageContentUnion]]] = Field(
        default=None,
        description="The content of the message.",
        json_schema_extra=get_riven_message_content_union_str_json_schema(),
    )
...
class Message(BaseMessage):
    """
        riven's internal representation of a message. Includes methods to convert to/from LLM provider formats.
    ...
    content: Optional[List[rivenMessageContentUnion]] = Field(default=None, description="The content of the message.")
    ...
    @staticmethod
    @trace_method
    def to_riven_messages_from_list(
        ...
        # Convert messages to rivenMessages
        riven_messages = [
            msg
            for m in messages
            for msg in m.to_riven_messages(
        ...
        if include_return_message_types is not None:
            ...
            riven_messages = [msg for msg in riven_messages if msg.message_type in include_return_message_types]

        return riven_messages

    @staticmethod
    @trace_method
    def to_riven_search_results_from_list(
        ...
        text_is_assistant_message: bool = False,
        convert_summary_to_user: bool = True,
    ) -> List[rivenMessageSearchResult]:
        """Convert MessageSearchResult objects into rivenMessageSearchResult objects.

        This mirrors the behavior of to_riven_messages_from_list, but preserves the
        originating Message.agent_id on each search result variant.
        """

        riven_search_results: List[rivenMessageSearchResult] = []

        for result in search_results:
            message = result.message

            # Convert the underlying Message into rivenMessage variants
            riven_messages = message.to_riven_messages(
                use_assistant_message=use_assistant_message,
                assistant_message_tool_name=assistant_message_tool_name,
                assistant_message_tool_kwarg=assistant_message_tool_kwarg,
                reverse=reverse,
            ...
            for lm in riven_messages:
                if isinstance(lm, SystemMessage):
                    riven_search_results.append(
                        SystemMessageListResult(
                ...
                # Other rivenMessage variants (tool, approval, etc.) are not part of
                # rivenMessageSearchResult and are intentionally skipped here.

        return riven_search_results

    def to_riven_messages(
        self,
        ...
        """Convert message object (in DB format) to the style used by the original riven API
    ...
    def _convert_explicit_tool_returns(self) -> ToolReturnMessage:
        ...
        for tool_return in self.tool_returns:
            ...
            tool_return_obj = rivenToolReturn(
    ...
    def dict_to_message(
        ...
        content: List[rivenMessageContentUnion] = (
    ...
    def _image_source_to_data_url(image_content: ImageContent) -> Optional[str]:
        ...
        if source.type == ImageSourceType.riven:
            data = getattr(source, "data", None)
            if not data:
                return None
            media_type = getattr(source, "media_type", None) or "image/png"
    ...
    def _get_base64_image_data(part: Union[ImageContent, dict]) -> Optional[tuple[str, str]]:
        ...
        if isinstance(part, ImageContent):
            ...
            elif source.type == ImageSourceType.riven and getattr(source, "data", None):
    ...
    def to_anthropic_dict(
        ...
        if self.role == "system":
            ...
            if system_text is None:
                text_parts = [c.text for c in (self.content or []) if isinstance(c, TextContent)]
                if not text_parts:
                    from riven.log import get_logger as _get_logger
...
class ToolReturn(BaseModel):
    ...
    func_response: Optional[Union[str, List[rivenToolReturnContentUnion]]] = Field(
