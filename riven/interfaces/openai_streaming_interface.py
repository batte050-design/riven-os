...
from riven.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from riven.llm_api.error_utils import is_context_window_overflow_message
from riven.llm_api.openai_client import is_openai_reasoning_model
...
from riven.schemas.openai.chat_completion_response import FunctionCall, ToolCall
from riven.server.rest_api.json_parser import OptimisticJSONParser
from riven.server.rest_api.streaming_response import RunCancelledException
from riven.server.rest_api.utils import decrement_message_uuid
from riven.services.context_window_calculator.token_counter import create_token_counter
from riven.streaming_utils import (
...
class OpenAIStreamingInterface:
    ...
    def __init__(
        ...
        self.riven_message_id = Message.generate_id()
    ...
    def get_usage_statistics(self) -> "rivenUsageStatistics":
        """Extract usage statistics from accumulated streaming data.

        Returns:
            rivenUsageStatistics with token counts from the stream.
        """
        from riven.schemas.usage import rivenUsageStatistics
    ...
    async def process(
    ...
    ) -> AsyncGenerator[rivenMessage | rivenStopReason, None]:
        """
        ...
        try:
            ...
            yield rivenStopReason(stop_reason=StopReasonType.error)
            raise e
        finally:
            ...
            if message_delta.tool_calls is not None and len(message_delta.tool_calls) > 0:
                ...
                if not self.emitted_hidden_reasoning and is_openai_reasoning_model(self.model) and not self.put_inner_thoughts_in_kwarg:
                    ...
                    hidden_message = HiddenReasoningMessage(
                        id=self.riven_message_id,
                        date=datetime.now(timezone.utc),
                        state="omitted",
                        hidden_reasoning=None,
                        otid=Message.generate_otid_from_id(self.riven_message_id, message_index),
                        run_id=self.run_id,
                            ...
                            if self.use_assistant_message and self._get_function_name_buffer() == self.assistant_message_tool_name:
                                ...
                                if self.tool_call_name in self.requires_approval_tools:
                                    tool_call_msg = ApprovalRequestMessage(
                                        id=decrement_message_uuid(self.riven_message_id),
                                        date=datetime.now(timezone.utc),
                                        tool_call=tool_call_delta,
                                        tool_calls=tool_call_delta,
                                        otid=Message.generate_otid_from_id(decrement_message_uuid(self.riven_message_id), -1),
                                        run_id=self.run_id,
                                        step_id=self.step_id,
                                    )
                                else:
...
class SimpleOpenAIResponsesStreamingInterface:
    ...
    def get_content(self) -> list[TextContent | SummarizedReasoningContent]:
        ...
        for response in self.final_response.output:
            if isinstance(response, ResponseReasoningItem):
                ...
                riven_summary = [SummarizedReasoningContentPart(index=i, text=part.text) for i, part in enumerate(response.summary)]
                content.append(
                    SummarizedReasoningContent(
                        id=response.id,
                        summary=riven_summary,
    ...
    async def _process_event(
        ...
        # Error / failure events (WebSocket mode may emit these explicitly)
            ...
            from riven.errors import LLMError
