...
from riven.constants import riven_MODEL_ENDPOINT, REQUEST_HEARTBEAT_PARAM
...
from riven.llm_api.error_utils import is_context_window_overflow_message, is_insufficient_credits_message
from riven.llm_api.helpers import (
    add_inner_thoughts_to_functions,
    convert_response_format_to_responses_api,
    unpack_all_inner_thoughts_from_kwargs,
)
from riven.llm_api.llm_client_base import LLMClientBase
from riven.llm_api.openai_ws_session import AsyncStreamCompat, OpenAIWSSessionManager
from riven.local_llm.constants import INNER_THOUGHTS_KWARG, INNER_THOUGHTS_KWARG_DESCRIPTION, INNER_THOUGHTS_KWARG_DESCRIPTION_GO_FIRST
from riven.log import get_logger
from riven.otel.tracing import trace_method
from riven.schemas.agent import AgentType
from riven.schemas.embedding_config import EmbeddingConfig
from riven.schemas.enums import ProviderCategory
from riven.schemas.riven_message_content import MessageContentType, TextContent
...
from riven.schemas.openai.responses_request import ResponsesRequest
from riven.schemas.response_format import JsonSchemaResponseFormat
...
class OpenAIClient(LLMClientBase):
    ...
    def _is_true_openai_request(self, llm_config: LLMConfig) -> bool:
        ...
        # Keep riven inference endpoint behavior unchanged.
        if llm_config.model_endpoint == riven_MODEL_ENDPOINT:
            return False
    ...
    def _apply_prompt_cache_settings(
        ...
        good cache affinity for riven agents, since each agent has a unique system
    ...
    def build_request_data_responses(
        ...
        if tools:
            ...
            # TODO relax this?
            if agent_type == AgentType.riven_v1_agent:
                new_tools = []
            ...
            # (temporal_riven_v1_agent_workflow.py or riven_agent_v3.py) before reaching here.
        ...
        if llm_config.model_endpoint == riven_MODEL_ENDPOINT:
            if not self.actor:
                # override user id for inference.riven.com
                import uuid
    ...
    def build_request_data(
        ...
        """
        # Shortcut for GPT-5 to use Responses API, but only for riven_v1_agent
        if use_responses_api(llm_config) and agent_type == AgentType.riven_v1_agent:
        ...
        if tools:  # only set tool_choice if tools exist
            if force_tool_call is not None:
                ...
                tool_choice = "required"
            elif self.requires_auto_tool_choice(llm_config) or agent_type == AgentType.riven_v1_agent:
                tool_choice = "auto"
            else:
    ...
    def extract_usage_statistics(self, response_data: dict | None, llm_config: LLMConfig) -> rivenUsageStatistics:
        """Extract usage statistics from OpenAI response and return as rivenUsageStatistics."""
        if not response_data:
            return rivenUsageStatistics()
    ...
    def handle_llm_error(self, e: Exception, llm_config: Optional[LLMConfig] = None) -> Exception:
        ...
        # trigger riven's context window compaction / retry logic.
        #
        # Example message:
