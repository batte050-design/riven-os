...
from riven.errors import rivenError
...
from riven.orm.sandbox_config import AgentEnvironmentVariable
from riven.orm.sqlalchemy_base import AccessType
from riven.otel.tracing import trace_method
from riven.prompts.prompt_generator import PromptGenerator
from riven.schemas.agent import (
    AgentState as PydanticAgentState,
    CreateAgent,
    InternalTemplateAgentCreate,
    UpdateAgent,
)
from riven.schemas.block import DEFAULT_BLOCKS, Block as PydanticBlock, BlockUpdate
from riven.schemas.embedding_config import EmbeddingConfig
from riven.schemas.enums import AgentType, PrimitiveType, TagMatchMode, ToolType, VectorDBProvider
from riven.schemas.environment_variables import AgentEnvironmentVariable as PydanticAgentEnvVar
from riven.schemas.file import FileMetadata as PydanticFileMetadata
from riven.schemas.group import Group as PydanticGroup, ManagerType
...
from riven.schemas.message import Message, Message as PydanticMessage, MessageCreate, MessageUpdate
from riven.schemas.passage import Passage as PydanticPassage
from riven.schemas.secret import Secret
from riven.schemas.source import Source as PydanticSource
from riven.schemas.tool import Tool as PydanticTool
from riven.schemas.tool_rule import ContinueToolRule, RequiresApprovalToolRule, TerminalToolRule
...
from riven.services.context_window_calculator.context_window_calculator import ContextWindowCalculator
...
from riven.services.helpers.agent_manager_helper import (
    _apply_filters,
...
class AgentManager:
    ...
    async def create_agent_async(
        ...
        if agent_create.agent_type == AgentType.riven_v1_agent:
        ...
        if agent_create.include_base_tools:
            ...
            elif agent_create.agent_type == AgentType.riven_v1_agent:
                tool_names |= calculate_base_tools(is_v2=True)
        ...
        async with db_registry.async_session() as session:
            async with session.begin():
                ...
                from riven.services.summarizer.summarizer_config import CompactionSettings, get_default_summarizer_model
    ...
    async def get_agent_archive_ids_async(self, agent_id: str, actor: PydanticUser) -> List[str]:
        ...
        from riven.orm import ArchivesAgents
    ...
    async def validate_agent_exists_async(self, agent_id: str, actor: PydanticUser) -> None:
        ...
        Raises:
            rivenAgentNotFoundError: If agent doesn't exist or user doesn't have access
        """
        async with db_registry.async_session() as session:
    ...
    async def attach_missing_files_tools_async(self, agent_state: PydanticAgentState, actor: PydanticUser) -> PydanticAgentState:
        ...
        attached_file_tool_names = {tool.name for tool in agent_state.tools if tool.tool_type == ToolType.riven_FILES_CORE}
        ...
        async with db_registry.async_session() as session:
            query = select(ToolModel).where(
                ...
                ToolModel.tool_type == ToolType.riven_FILES_CORE,
            )
            result = await session.execute(query)
    ...
    async def detach_all_files_tools_async(self, agent_state: PydanticAgentState, actor: PydanticUser) -> PydanticAgentState:
        ...
        file_tool_ids = [tool.id for tool in agent_state.tools if tool.tool_type == ToolType.riven_FILES_CORE]
        ...
        # create a shallow copy with updated tools list to avoid modifying input
        agent_state_dict = agent_state.model_dump()
        agent_state_dict["tools"] = [tool for tool in agent_state.tools if tool.tool_type != ToolType.riven_FILES_CORE]
