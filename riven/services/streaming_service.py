...
from riven.constants import REDIS_RUN_ID_PREFIX
from riven.data_sources.redis_client import AsyncRedisClient, NoopAsyncRedisClient, get_redis_client
...
from riven.helpers.datetime_helpers import get_utc_timestamp_ns
...
from riven.schemas.enums import AgentType, MessageStreamStatus, RunStatus
from riven.schemas.job import rivenRequestConfig
from riven.schemas.riven_message import AssistantMessage, rivenErrorMessage, rivenPing, MessageType
from riven.schemas.riven_message_content import TextContent
from riven.schemas.riven_request import ClientToolSchema, rivenStreamingRequest
...
from riven.schemas.message import MessageCreate
...
from riven.server.rest_api.redis_stream_manager import create_background_stream_processor, redis_sse_stream_generator
from riven.server.rest_api.streaming_response import (
...
from riven.server.rest_api.utils import capture_sentry_exception
...
from riven.utils import safe_create_task
...
class StreamingService:
    ...
    async def create_agent_stream(
        ...
        Args:
            ...
            request: The rivenStreamingRequest containing all request parameters
        ...
        if lock_key and not isinstance(redis_client, NoopAsyncRedisClient):
            ...
            try:
                ...
                from riven.monitoring.load_gate import get_load_gate
        ...
        try:
            ...
            if request.background and settings.track_agent_run:
                if isinstance(redis_client, NoopAsyncRedisClient):
                    raise rivenServiceUnavailableError(
                        f"Background streaming requires Redis to be running. "
                        f"Please ensure Redis is properly configured. "
                        f"riven_REDIS_HOST: {settings.redis_host}, riven_REDIS_PORT: {settings.redis_port}",
                        service_name="redis",
                    )
    ...
    async def create_agent_stream_openai_chat_completions(
        ...
        Transforms riven's internal streaming format to match OpenAI's
        ...
        # create standard riven stream (returns SSE-formatted stream)
        run, riven_stream_response = await self.create_agent_stream(
            agent_id=agent_id,
            actor=actor,
            request=request,
        ...
        if isinstance(riven_stream_response, StreamingResponseWithStatusCode):
            riven_stream = riven_stream_response.body_iterator
        else:
        ...
        # transform riven SSE stream to OpenAI format (parser handles SSE strings)
        openai_stream = transformer.transform_stream(riven_stream)
        ...
        async def error_aware_stream():
            ...
            try:
                ...
                if not saw_done and not saw_error:
                    ...
                    stop_reason = rivenStopReason(stop_reason=StopReasonType.error)
                    ...
                    stop_reason = agent_loop.stop_reason if agent_loop.stop_reason else rivenStopReason(stop_reason=StopReasonType.end_turn)
                ...
                run_status = RunStatus.failed
                stop_reason = rivenStopReason(stop_reason=StopReasonType.llm_api_error)
                error_message = rivenErrorMessage(
                    run_id=run_id,
                ...
                run_status = RunStatus.failed
                stop_reason = rivenStopReason(stop_reason=StopReasonType.invalid_llm_response)
                error_message = rivenErrorMessage(
                    run_id=run_id,
                ...
                run_status = RunStatus.failed
                stop_reason = rivenStopReason(stop_reason=StopReasonType.context_window_overflow_in_system_prompt)
                ...
                error_message = rivenErrorMessage(
                    ...
                    message=error_detail if isinstance(e, rivenError) else "An unknown error occurred with the LLM streaming request.",
                    detail=error_detail,
                )
                error_data = {"error": error_message.model_dump()}
                ...
                if run_id and self.runs_manager and run_status:
                    # Extract stop_reason enum value from rivenStopReason object
    ...
    def _is_token_streaming_compatible(self, agent: AgentState) -> bool:
        ...
        google_riven_v1 = agent.agent_type == AgentType.riven_v1_agent and agent.llm_config.model_endpoint_type in [
            "google_ai",
            "google_vertex",
        ]
        return base_compatible or google_riven_v1
    ...
    def _extract_sse_error_type(chunk: str) -> Optional[str]:
        """Extract riven stream error_type from an SSE error chunk."""
        if not ("\nevent: error" in chunk or chunk.startswith("event: error")):
            return None
    ...
    async def _create_run(
        self, agent_id: str, request: rivenStreamingRequest, run_type: str, actor: User, conversation_id: Optional[str] = None
    ) -> PydanticRun:
...
class OpenAIChatCompletionsStreamTransformer:
    """
    Transforms riven streaming messages into OpenAI ChatCompletionChunk format.
        ...
        try:
            ...
            if sse_string.startswith("data: "):
                ...
                if message_type == "assistant_message":
                    ...
                    return rivenUsageStatistics(**data)
    ...
    async def transform_stream(self, riven_stream: AsyncIterator) -> AsyncIterator[str]:
        """
        Transform riven stream to OpenAI ChatCompletionChunk SSE format.

        Args:
            riven_stream: Async iterator of riven messages (may be SSE strings or objects)

        Yields:
            SSE-formatted strings: "data: {json}\n\n"
        """
        try:
            async for raw_chunk in riven_stream:
    ...
    async def _process_assistant_message(self, message: AssistantMessage) -> AsyncIterator[str]:
        ...
        Args:
            message: riven AssistantMessage with content
