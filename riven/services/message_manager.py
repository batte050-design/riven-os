...
from riven.constants import CONVERSATION_SEARCH_TOOL_NAME, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from riven.log import get_logger
from riven.orm.conversation_messages import ConversationMessage
from riven.orm.errors import NoResultFound
from riven.orm.message import Message as MessageModel
from riven.otel.tracing import trace_method
from riven.schemas.enums import MessageRole, PrimitiveType
from riven.schemas.riven_message import rivenMessageUpdateUnion
from riven.schemas.riven_message_content import ImageSourceType, rivenImage, MessageContentType
from riven.schemas.message import Message as PydanticMessage, MessageSearchResult, MessageUpdate
...
from riven.utils import enforce_types, fire_and_forget
...
class MessageManager:
    ...
    def _combine_assistant_tool_messages(self, messages: List[PydanticMessage]) -> List[PydanticMessage]:
        ...
        """
        from riven.constants import DEFAULT_MESSAGE_TOOL
    ...
    async def create_many_messages_async(
        ...
        from riven.helpers.tpuf_client import should_use_tpuf_for_messages
    ...
    @enforce_types
    @trace_method
    async def update_message_by_riven_message_async(
        self, message_id: str, riven_message_update: rivenMessageUpdateUnion, actor: PydanticUser
    ) -> PydanticMessage:
        """
        Updated the underlying messages table giving an update specified to the user-facing rivenMessage
        """
        message = await self.get_message_by_id_async(message_id=message_id, actor=actor)
        if riven_message_update.message_type == "assistant_message":
            ...
            original_args["message"] = riven_message_update.content  # override the assistant message
        ...
        elif riven_message_update.message_type == "reasoning_message":
            update_message = MessageUpdate(content=riven_message_update.reasoning)
        elif riven_message_update.message_type == "user_message" or riven_message_update.message_type == "system_message":
            update_message = MessageUpdate(content=riven_message_update.content)
        else:
            raise ValueError(f"Unsupported message type for modification: {riven_message_update.message_type}")

        message = await self.update_message_by_id_async(message_id=message_id, message_update=update_message, actor=actor)

        # convert back to rivenMessage
        for riven_msg in message.to_riven_messages(use_assistant_message=True):
            if riven_msg.message_type == riven_message_update.message_type:
                return riven_msg

        # raise error if message type got modified
        raise ValueError(f"Message type got modified: {riven_message_update.message_type}")
    ...
    async def delete_message_by_id_async(self, message_id: str, actor: PydanticUser, strict_mode: bool = False) -> bool:
        ...
        from riven.helpers.tpuf_client import TurbopufferClient, should_use_tpuf_for_messages
