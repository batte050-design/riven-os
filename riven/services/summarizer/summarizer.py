...
from riven.errors import ContextWindowExceededError, LLMProviderOverloaded, LLMRateLimitError
...
from riven.schemas.enums import AgentType, LLMCallType, MessageRole, ProviderCategory, ProviderType
...
from riven.system import package_summarize_message_no_counts
...
class Summarizer:
    ...
    async def summarize(
        ...
        Args:
            ...
            new_riven_messages: The newly added riven messages (just appended).
    ...
    async def _partial_evict_buffer_summarization(
        ...
        """
        all_in_context_messages = in_context_messages + new_riven_messages
        ...
        from riven.settings import summarizer_settings
    ...
    def _static_buffer_summarization(
        ...
        Args:
            ...
            new_riven_messages: Newly added messages to append
...
async def simple_summary(
    ...
    """
    from riven.prompts.summarizer_prompt import ALL_PROMPT
    from riven.services.telemetry_manager import TelemetryManager
    ...
    request_data = llm_client.build_request_data(AgentType.riven_v1_agent, input_messages_obj, summarizer_llm_config, tools=[])
    try:
...
async def _get_summarizer_fallback_config(llm_config: LLMConfig, actor: User) -> LLMConfig | None:
    ...
    if endpoint_type in ("zai", "zai_coding"):
        from riven.services.llm_router.llm_router_client import _build_baseten_config
...
async def _run_summarizer_request(
    ...
    try:
        ...
        try:
            ...
            fallback_req = fallback_client.build_request_data(AgentType.riven_v1_agent, req_messages_obj, fallback_llm_config, tools=[])
...
async def _execute_summarizer_request(req_data: dict, req_messages_obj: list[Message], llm_config: LLMConfig, llm_client: LLMClient) -> str:
    ...
    if llm_config.model_endpoint_type in [ProviderType.anthropic, ProviderType.bedrock]:
        ...
        from riven.interfaces.anthropic_parallel_tool_call_streaming_interface import (
