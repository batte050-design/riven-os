...
from riven.services.summarizer.self_summarizer import self_summarize_all, self_summarize_sliding_window
from riven.services.summarizer.summarizer_all import summarize_all
from riven.services.summarizer.summarizer_config import CompactionSettings, get_default_prompt_for_mode, get_default_summarizer_model
from riven.services.summarizer.summarizer_sliding_window import (
    count_tokens,
...
async def build_summarizer_llm_config(
    ...
    if agent_llm_config.handle and agent_llm_config.handle.startswith("riven/auto"):
    ...
    try:
        ...
        # (safety net for stale compaction_settings that still reference riven/auto)
        if summarizer_config.model and summarizer_config.model.startswith("riven/auto"):
            try:
    ...
    if trigger_threshold is not None and context_token_estimate is not None and context_token_estimate >= trigger_threshold:
        ...
        if context_token_estimate is not None and context_token_estimate >= trigger_threshold:
            ...
            if system_prompt_token_estimate is not None and system_prompt_token_estimate >= agent_llm_config.context_window:
                from riven.errors import SystemPromptTokenExceededError
