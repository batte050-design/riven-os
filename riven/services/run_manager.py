...
from riven.log_context import update_log_context
from riven.orm.agent import Agent as AgentModel
from riven.orm.errors import NoResultFound
from riven.orm.run import Run as RunModel
from riven.orm.run_metrics import RunMetrics as RunMetricsModel
from riven.orm.sqlalchemy_base import AccessType
from riven.otel.tracing import log_event, trace_method
from riven.schemas.enums import AgentType, ComparisonOperator, MessageRole, PrimitiveType, RunStatus
from riven.schemas.job import rivenRequestConfig
from riven.schemas.riven_message import rivenMessage
...
from riven.schemas.message import Message as PydanticMessage
from riven.schemas.run import Run as PydanticRun, RunUpdate
from riven.schemas.run_metrics import RunMetrics as PydanticRunMetrics
from riven.schemas.step import Step as PydanticStep
from riven.schemas.usage import rivenUsageStatistics, normalize_cache_tokens, normalize_reasoning_tokens
...
from riven.services.helpers.agent_manager_helper import validate_agent_exists_async
from riven.services.message_manager import MessageManager
from riven.services.step_manager import StepManager
from riven.utils import enforce_types
...
class RunManager:
    ...
    async def list_runs(
        ...
        async with db_registry.async_session() as session:
            ...
            if order_by == "duration":
                ...
                from riven.services.helpers.run_manager_helper import _apply_pagination_async
    ...
    async def update_run_by_id_async(
        ...
        if is_terminal_update and update.stop_reason:
            try:
                from riven.schemas.agent import UpdateAgent
        ...
        if needs_callback:
            if refresh_result_messages:
                ...
                result = rivenResponse(
                    ...
                    stop_reason=rivenStopReason(stop_reason=stop_reason_value),
    ...
    @enforce_types
    @raise_on_invalid_id(param_name="run_id", expected_prefix=PrimitiveType.RUN)
    async def get_run_usage(self, run_id: str, actor: PydanticUser) -> rivenUsageStatistics:
        ...
        total_usage = rivenUsageStatistics()
        for step in steps:
    ...
    async def get_run_messages(
        ...
        text_is_assistant_message = agent.agent_type == AgentType.riven_v1_agent
        ...
        riven_messages = PydanticMessage.to_riven_messages_from_list(
        ...
        if request_config and request_config.include_return_message_types:
            ...
            riven_messages = [msg for msg in riven_messages if msg.message_type in include_return_message_types_set]

        return riven_messages

    @enforce_types
    @raise_on_invalid_id(param_name="run_id", expected_prefix=PrimitiveType.RUN)
    async def get_run_request_config(self, run_id: str, actor: PydanticUser) -> Optional[rivenRequestConfig]:
        """Get the riven request config from a run."""
        async with db_registry.async_session() as session:
            run = await RunModel.read_async(db_session=session, identifier=run_id, actor=actor, access_type=AccessType.ORGANIZATION)
            if not run:
                raise NoResultFound(f"Run with id {run_id} not found")
    ...
    async def cancel_run(self, actor: PydanticUser, agent_id: Optional[str] = None, run_id: Optional[str] = None) -> None:
        ...
        if was_pending_approval:
            ...
            if all_pending_tool_calls:
                from riven.constants import TOOL_CALL_DENIAL_ON_CANCEL
                from riven.schemas.riven_message import ApprovalReturn
                from riven.schemas.message import ApprovalCreate
