...
from riven.constants import (
    ...
    BUILTIN_TOOLS,
    FILES_TOOLS,
    riven_PARALLEL_SAFE_TOOLS,
    riven_TOOL_MODULE_NAMES,
    riven_TOOL_SET,
...
from riven.errors import rivenInvalidArgumentError, rivenToolNameConflictError, rivenToolNameSchemaMismatchError
from riven.functions.functions import load_function_set
from riven.helpers.tool_helpers import compute_tool_hash, generate_modal_function_name
from riven.log import get_logger

# TODO: Remove this once we translate all of these to the ORM
from riven.orm.errors import NoResultFound
from riven.orm.tool import Tool as ToolModel
from riven.otel.tracing import trace_method, tracer
from riven.schemas.enums import PrimitiveType, SandboxType, ToolType
from riven.schemas.tool import Tool as PydanticTool, ToolCreate, ToolUpdate
from riven.schemas.user import User as PydanticUser
from riven.server.db import db_registry
from riven.services.helpers.agent_manager_helper import calculate_multi_agent_tools
from riven.services.mcp.types import SSEServerConfig, StdioServerConfig
from riven.services.tool_schema_generator import generate_schema_for_tool_creation, generate_schema_for_tool_update
from riven.settings import settings, tool_settings
from riven.utils import enforce_types, fire_and_forget, printd
from riven.validators import raise_on_invalid_id
...
def modal_tool_wrapper(tool: PydanticTool, actor: PydanticUser, sandbox_env_vars: dict | None = None, project_id: str = "default"):
    ...
    packages.append("riven_client>=1.1.1")
    packages.append("riven")  # Base riven without extras
    ...
    secrets_dict = {"riven_API_KEY": None}
    if sandbox_env_vars:
    ...
    def modal_function(
        tool_name: str, agent_state: Optional[dict], agent_id: Optional[str], env_vars: dict, riven_api_key: Optional[str] = None, **kwargs
    ):
        ...
        if agent_state:
            try:
                from riven.schemas.agent import AgentState as AgentStateModel
        ...
        # TODO: directly instantiate the riven client once we upgrade to 1.0.0+ in core
        # Initialize the riven client
        # if riven_api_key:
        #     riven_client = riven(token=riven_api_key, base_url=os.environ.get("riven_API_URL", "https://api.riven.com"))
        # else:
        #     riven_client = None

        tool_namespace = {
            "__builtins__": __builtins__,  # Include built-in functions
            # "_riven_client": riven_client,  # Make riven_client available
            "os": os,  # Include os module for env vars access
            "agent_id": agent_id,
            "_riven_API_KEY": riven_api_key,
        ...
        with contextlib.redirect_stdout(stdout_capture), contextlib.redirect_stderr(stderr_capture):
            try:
                ...
                try:
                    from riven.functions.ast_parsers import coerce_dict_args_by_annotations
        ...
        return {
            ...
            "stdout": stdout,
            "stderr": stderr,
            "agent_state": agent_state,  # TODO: deprecate (use riven_client instead)
...
class ToolManager:
    ...
    async def create_or_update_tool_async(
        ...
        if not pydantic_tool.name:
            ...
            if pydantic_tool.name != pydantic_tool.json_schema.get("name"):
                raise rivenToolNameSchemaMismatchError(
        ...
        if settings.riven_pg_uri_no_default:
    ...
    async def _atomic_upsert_tool_postgresql(
        ...
        from riven.helpers.tpuf_client import should_use_tpuf_for_tools
    ...
    async def list_tools_async(
        ...
        return_only_riven_tools: bool = False,
        ...
        tools = await self._list_tools_async(
            ...
            tool_ids=tool_ids,
            search=search,
            return_only_riven_tools=return_only_riven_tools,
        ...
        if not after and upsert_base_tools:
            ...
            base_tool_names = riven_TOOL_SET - set(LOCAL_ONLY_MULTI_AGENT_TOOLS) if settings.environment == "prod" else riven_TOOL_SET
    ...
    async def _list_tools_async(
        ...
        async with db_registry.async_session() as session:
            ...
            # Apply return_only_riven_tools filter at database level
            if return_only_riven_tools:
                query = query.where(ToolModel.tool_type.like("riven_%"))
    ...
    async def count_tools_async(
        ...
        search: Optional[str] = None,
        return_only_riven_tools: bool = False,
        exclude_riven_tools: bool = False,
        project_id: Optional[str] = None,
    ) -> int:
        ...
        async with db_registry.async_session() as session:
            ...
            # Handle exclude_riven_tools logic (if True, exclude riven tools)
            if exclude_riven_tools:
                # Exclude tools that are in the riven_TOOL_SET
                riven_tool_names = list(riven_TOOL_SET)
                query = query.where(~ToolModel.name.in_(riven_tool_names))
    ...
    async def size_async(
        ...
        async with db_registry.async_session() as session:
            ...
            return await ToolModel.size_async(db_session=session, actor=actor, name=riven_TOOL_SET)
    ...
    async def update_tool_by_id_async(
        ...
        async with db_registry.async_session() as session:
            ...
            if needs_name_conflict_check:
                ...
                if name_conflict:
                    raise rivenToolNameConflictError(tool_name=new_name)
        ...
        if should_use_tpuf_for_tools():
            ...
            async def update_tool_embedding():
                try:
                    from riven.helpers.tpuf_client import TurbopufferClient
    ...
    async def upsert_base_tools_async(
        ...
        functions_to_schema = {}
        for module_name in riven_TOOL_MODULE_NAMES:
            try:
        ...
        for name, schema in functions_to_schema.items():
            if name not in riven_TOOL_SET:
                continue

            if name in BASE_TOOLS:
                tool_type = ToolType.riven_CORE
            elif name in BASE_MEMORY_TOOLS:
                tool_type = ToolType.riven_MEMORY_CORE
            elif name in BASE_SLEEPTIME_TOOLS:
                tool_type = ToolType.riven_SLEEPTIME_CORE
            elif name in calculate_multi_agent_tools():
                tool_type = ToolType.riven_MULTI_AGENT_CORE
            elif name in BASE_VOICE_SLEEPTIME_TOOLS or name in BASE_VOICE_SLEEPTIME_CHAT_TOOLS:
                tool_type = ToolType.riven_VOICE_SLEEPTIME_CORE
            elif name in BUILTIN_TOOLS:
                tool_type = ToolType.riven_BUILTIN
            elif name in FILES_TOOLS:
                tool_type = ToolType.riven_FILES_CORE
            else:
            ...
            parallel_safe = name in riven_PARALLEL_SAFE_TOOLS
    ...
    async def create_or_update_modal_app(self, tool: PydanticTool, actor: PydanticUser):
        ...
        with modal.enable_output(show_progress=False):
            try:
                ...
                raise rivenInvalidArgumentError(f"Failed to deploy tool {tool.id} with name {tool.name} to Modal: {e}")
    ...
    async def search_tools_async(
        ...
        """
        from riven.helpers.tpuf_client import TurbopufferClient, should_use_tpuf_for_tools
