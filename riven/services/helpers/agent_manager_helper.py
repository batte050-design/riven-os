...
from riven.errors import rivenAgentNotFoundError
from riven.helpers import ToolRulesSolver
from riven.helpers.datetime_helpers import get_local_time
from riven.llm_api.llm_client import LLMClient
from riven.orm.agent import Agent as AgentModel
from riven.orm.agents_tags import AgentsTags
from riven.orm.archives_agents import ArchivesAgents
...
from riven.orm.sources_agents import SourcesAgents
from riven.otel.tracing import trace_method
from riven.prompts import gpt_system
...
from riven.schemas.enums import AgentType, MessageRole
...
from riven.system import get_initial_boot_messages, get_login_event, package_function_response
...
def derive_system_message(agent_type: AgentType, enable_sleeptime: Optional[bool] = None, system: Optional[str] = None) -> str:
    ...
    if system is None:
        ...
        # riven v1
        elif agent_type == AgentType.riven_v1_agent:
            system = gpt_system.get_system_text("riven_v1")
...
def initialize_message_sequence(
    ...
    first_user_message = get_login_event(agent_state.timezone)  # event letting riven know the user just logged in
...
async def initialize_message_sequence_async(
    ...
    if agent_state.agent_type == AgentType.riven_v1_agent:
    ...
    if include_initial_boot_message:
        ...
        if agent_state.agent_type == AgentType.sleeptime_agent or agent_state.agent_type == AgentType.riven_v1_agent:
            initial_boot_messages = []
        elif llm_config.model is not None and "gpt-3.5" in llm_config.model:
            initial_boot_messages = get_initial_boot_messages("startup_with_send_message_gpt35", agent_state.timezone, tool_call_id)
        else:
...
async def build_passage_query(
    ...
    if embedded_text:
        if settings.database_engine is DatabaseChoice.POSTGRES:
            ...
            from riven.orm.sqlite_functions import adapt_array
...
def calculate_base_tools(is_v2: bool) -> Set[str]:
    if is_v2:
        return (set(BASE_TOOLS) - set(DEPRECATED_riven_TOOLS)) | set(BASE_MEMORY_TOOLS_V2)
    else:
        return (set(BASE_TOOLS) - set(DEPRECATED_riven_TOOLS)) | set(BASE_MEMORY_TOOLS)
...
async def validate_agent_exists_async(session, agent_id: str, actor: User) -> None:
    ...
    if not result.scalar():
        raise rivenAgentNotFoundError(f"Agent with ID {agent_id} not found")
