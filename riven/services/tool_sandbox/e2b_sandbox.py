...
from riven.services.helpers.tool_parser_helper import parse_stdout_best_effort
from riven.services.tool_sandbox.base import AsyncToolSandboxBase
from riven.services.tool_sandbox.typescript_generator import parse_typescript_result
...
class AsyncToolSandboxE2B(AsyncToolSandboxBase):
    ...
    async def create_e2b_sandbox_with_metadata_hash(self, sandbox_config: SandboxConfig) -> "AsyncSandbox":
        ...
        # Auto-install @riven-ai/riven-client for TypeScript tools (similar to riven_client for Python)
        if self.is_typescript_tool():
            riven_client_package = "@riven-ai/riven-client"
            log_event(
                "riven_client_npm_install_started",
                {
                    "sandbox_id": sbx.sandbox_id,
                    "package": riven_client_package,
                    "tool_name": self.tool.name if self.tool else None,
                },
            )
            try:
                await sbx.commands.run(f"npm install {riven_client_package}")
                log_event(
                    "riven_client_npm_install_finished",
                    {
                        "sandbox_id": sbx.sandbox_id,
                        "package": riven_client_package,
                        "tool_name": self.tool.name if self.tool else None,
                ...
                logger.warning(f"Failed to install {riven_client_package} in E2B sandbox: {e}")
                log_event(
                    "riven_client_npm_install_failed",
                    {
                        "sandbox_id": sbx.sandbox_id,
                        "package": riven_client_package,
                        "tool_name": self.tool.name if self.tool else None,
