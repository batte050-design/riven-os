...
def generate_typescript_execution_script(
    ...
    1. Imports and initializes the riven client (available as `client` variable)
    ...
    The agent_id is available via process.env.riven_AGENT_ID environment variable.
    ...
    # Import and initialize riven client (similar to Python's riven_client injection)
    ...
    lines.extend(
        [
            "// Initialize riven client for TypeScript tool execution",
            "let client: any = null;",
            "try {",
            "    const { rivenClient } = await import('@riven-ai/riven-client');",
            "    const apiKey = process.env.riven_API_KEY;",
            "    if (apiKey) {",
            "        client = new rivenClient({ apiKey });",
            "    }",
            "} catch (e) {",
