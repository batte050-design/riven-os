...
from riven.constants import riven_MULTI_AGENT_TOOL_MODULE_NAME
from riven.functions.helpers import generate_model_from_args_json_schema
from riven.otel.tracing import trace_method
from riven.schemas.agent import AgentState
from riven.schemas.enums import ToolSourceType, ToolType
...
from riven.services.helpers.tool_execution_helper import add_imports_and_pydantic_schemas_for_args
from riven.services.helpers.tool_parser_helper import convert_param_to_str_value, parse_function_arguments
...
from riven.types import JsonDict, JsonValue
...
class AsyncToolSandboxBase(ABC):
    ...
    async def _init_async(self):
        ...
        if not self._initialized:
            ...
            if not self.tool.source_code and self.tool.tool_type == ToolType.riven_MULTI_AGENT_CORE:
                module = importlib.import_module(riven_MULTI_AGENT_TOOL_MODULE_NAME)
            ...
            # (these are Python-only features). Instead, agent_id is exposed via riven_AGENT_ID env var.
            ...
            # Always inject riven client (available as `client` variable in sandbox)
            # For Python: riven_client package
            # For TypeScript: @riven-ai/riven-client package
            self.inject_riven_client = True
    ...
    async def generate_execution_script(self, agent_state: Optional[AgentState], wrap_print_with_markers: bool = False) -> str:
        ...
        code = self._render_sandbox_code(
            ...
            inject_riven_client=self.inject_riven_client,
    ...
    def _render_sandbox_code(
        ...
        *,
        ...
        if inject_agent_state:
            lines.extend(["import riven", "from riven import *"])

        # Import riven client if available (wrapped in try/except for sandboxes without riven_client installed)
        if inject_riven_client:
            lines.extend(
                [
                    "try:",
        ...
        # Initialize riven client if needed (client is always available as a variable, may be None)
        if inject_riven_client:
            lines.extend(
                [
                    "# Initialize riven client for tool execution",
                    "import os",
                    "client = None",
                    "if riven is not None and os.getenv('riven_API_KEY'):",
                    "    # Check riven_client version to use correct parameter name",
                    "    from packaging import version as pkg_version",
                    "    import riven_client as lc_module",
                    "    lc_version = pkg_version.parse(lc_module.__version__)",
                    "    if lc_version < pkg_version.parse('1.0.0'):",
                    "        client = riven(",
                    "            token=os.getenv('riven_API_KEY')",
                    "        )",
                    "    else:",
                    "        client = riven(",
        ...
        if self.args:
            ...
            lines.extend(
                [
                    f"__riven_raw_args = {{{raw_args}}}",
                    "try:",
                    "    from riven.functions.ast_parsers import coerce_dict_args_by_annotations",
                    f"    __riven_func = {self.tool.name}",
                    "    __riven_annotations = getattr(__riven_func, '__annotations__', {})",
                    "    __riven_coerced_args = coerce_dict_args_by_annotations(",
                    "        __riven_raw_args,",
                    "        __riven_annotations,",
                    "        allow_unsafe_eval=True,",
                    "        extra_globals=__riven_func.__globals__,",
                    "    )",
                ]
            )
            for name in self.args:
                lines.append(f"    {name} = __riven_coerced_args.get({name!r}, {name})")
    ...
    async def _generate_typescript_execution_script(self, agent_state: Optional[AgentState]) -> str:
        ...
        - agent_id is available via process.env.riven_AGENT_ID
        - Return JSON-serialized results instead of pickle
        - Require explicit json_schema (no docstring parsing)
        """
        from riven.services.tool_sandbox.typescript_generator import generate_typescript_execution_script
    ...
    async def _gather_env_vars(
        ...
        if self.agent_id:
            env["riven_AGENT_ID"] = self.agent_id
        if self.project_id:
            env["riven_PROJECT_ID"] = self.project_id
        env["riven_TOOL_ID"] = self.tool_id
