...
from riven.jobs.types import BatchPollingResult, ItemUpdateInfo, RequestStatusUpdateInfo, StepStatusUpdateInfo
from riven.log import get_logger
from riven.orm import Message as MessageModel
...
from riven.schemas.enums import AgentStepStatus, JobStatus, ProviderType
from riven.schemas.llm_batch_job import AgentStepState, LLMBatchItem as PydanticLLMBatchItem, LLMBatchJob as PydanticLLMBatchJob
...
class LLMBatchManager:
    ...
    async def list_llm_batch_jobs_async(
        ...
        """
        async with db_registry.async_session() as session:
            query = select(LLMBatchJob).where(LLMBatchJob.riven_batch_job_id == riven_batch_id)
    ...
    @enforce_types
    @trace_method
    async def get_messages_for_riven_batch_async(
        self,
        riven_batch_job_id: str,
        actor: PydanticUser,
        limit: int = 100,
        ...
        """
        Retrieve messages across all LLM batch jobs associated with a riven batch job.
        ...
        async with db_registry.async_session() as session:
            ...
            query = (
                ...
                .where(LLMBatchJob.riven_batch_job_id == riven_batch_job_id)
