...
from riven.orm.block_history import BlockHistory
from riven.orm.blocks_agents import BlocksAgents
from riven.orm.blocks_tags import BlocksTags
...
from riven.schemas.block import Block as PydanticBlock, BlockUpdate
from riven.schemas.enums import ActorType, PrimitiveType
...
from riven.utils import bounded_gather, decrypt_agent_secrets, enforce_types
...
class BlockManager:
    ...
    async def checkpoint_block_async(
        ...
        async with db_registry.async_session() as session:
            ...
            history_entry = BlockHistory(
                ...
                actor_type=ActorType.riven_AGENT if agent_id else ActorType.riven_USER,
    ...
    async def undo_checkpoint_block(
        ...
        async with db_registry.async_session() as session:
            ...
            if not block.current_history_entry_id:
                raise rivenInvalidArgumentError(f"Block {block_id} has no history entry - cannot undo.", argument_name="block_id")
    ...
    async def redo_checkpoint_block(
        ...
        async with db_registry.async_session() as session:
            ...
            if not block.current_history_entry_id:
                raise rivenInvalidArgumentError(f"Block {block_id} has no history entry - cannot redo.", argument_name="block_id")
