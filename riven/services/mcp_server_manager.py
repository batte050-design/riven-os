...
from riven.functions.mcp_client.types import (
    MCPServerType,
    MCPTool,
    MCPToolHealth,
    SSEServerConfig,
...
from riven.functions.schema_generator import normalize_mcp_schema
from riven.functions.schema_validator import validate_complete_json_schema
from riven.helpers.url_validation import validate_mcp_server_url
from riven.log import get_logger
from riven.orm.errors import NoResultFound
from riven.orm.mcp_oauth import MCPOAuth, OAuthSessionStatus
from riven.orm.mcp_server import MCPServer as MCPServerModel, MCPTools as MCPToolsModel
from riven.orm.tool import Tool as ToolModel
from riven.schemas.mcp import (
    MCPOAuthSession,
...
from riven.schemas.mcp_server import CreateMCPServerRequest, CreateSSEMCPServer, CreateStdioMCPServer, CreateStreamableHTTPMCPServer
...
from riven.services.mcp.fastmcp_client import AsyncFastMCPSSEClient, AsyncFastMCPStreamableHTTPClient
from riven.services.mcp.server_side_oauth import ServerSideOAuth
...
from riven.utils import enforce_types, printd, safe_create_task
...
class MCPServerManager:
    ...
    async def add_tool_from_mcp_server(self, mcp_server_id: str, mcp_tool_name: str, actor: PydanticUser) -> PydanticTool:
        """Add a tool from an MCP server to the riven tool registry."""
    ...
    async def handle_oauth_flow(
        ...
        from riven.services.mcp.oauth_utils import oauth_stream_event
        ...
        # Check if request is from riven-code CLI (uses web callback for OAuth)
        is_riven_code_request = http_request and http_request.headers and http_request.headers.get("x-riven-source", "") == "riven-code"

        logo_uri = None
        NEXT_PUBLIC_CURRENT_HOST = os.getenv("NEXT_PUBLIC_CURRENT_HOST")
        riven_AGENTS_ENDPOINT = os.getenv("riven_AGENTS_ENDPOINT")

        if (is_web_request or is_riven_code_request) and NEXT_PUBLIC_CURRENT_HOST:
        ...
        elif riven_AGENTS_ENDPOINT:
            # API and SDK usage should call core server directly
            # Use static callback URI - session is identified via state parameter
            redirect_uri = f"{riven_AGENTS_ENDPOINT}/v1/tools/mcp/oauth/callback"
        else:
            logger.error(
                f"No redirect URI found for request and base urls: {http_request.headers if http_request else 'No headers'} {NEXT_PUBLIC_CURRENT_HOST} {riven_AGENTS_ENDPOINT}"
            )
