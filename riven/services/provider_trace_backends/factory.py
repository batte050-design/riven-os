...
from riven.services.provider_trace_backends.base import ProviderTraceBackend, ProviderTraceBackendClient
...
def _create_backend(backend: ProviderTraceBackend | str) -> ProviderTraceBackendClient:
    ...
    match backend_str:
        case "clickhouse":
            from riven.services.provider_trace_backends.clickhouse import ClickhouseProviderTraceBackend

            return ClickhouseProviderTraceBackend()

        case "socket":
            from riven.services.provider_trace_backends.socket import SocketProviderTraceBackend

            return SocketProviderTraceBackend(socket_path=telemetry_settings.socket_path)

        case "postgres" | _:
            from riven.services.provider_trace_backends.postgres import PostgresProviderTraceBackend
