...
from riven.schemas.memory_repo import FileChange, MemoryCommit
...
class GitOperations:
    ...
    async def create_repo(
        ...
        author_name: str = "riven System",
        author_email: str = "system@riven.ai",
    ) -> str:
        ...
        def _create():
            temp_dir = tempfile.mkdtemp(prefix="riven-memrepo-")
            try:
                ...
                if initial_files:
                    ...
                    # Create an empty .riven directory to initialize
                    riven_dir = os.path.join(repo_path, ".riven")
                    os.makedirs(riven_dir, exist_ok=True)
                    config_path = os.path.join(riven_dir, "config.json")
                    with open(config_path, "w") as f:
                        f.write('{"version": 1}')
                    _run_git(["add", ".riven/config.json"], cwd=repo_path)
    ...
    async def commit(
        ...
        message: str,
        author_name: str = "riven Agent",
        author_email: str = "agent@riven.ai",
        branch: str = "main",
    ) -> MemoryCommit:
