...
from riven.log_context import get_log_context
from riven.settings import log_settings, settings, telemetry_settings
...
class JSONFormatter(logging.Formatter):
    ...
    Usage:
        Enable JSON logging by setting the environment variable:
            riven_LOGGING_JSON_LOGGING=true
...
class LogContextFilter(logging.Filter):
    ...
    Usage:
        from riven.log_context import set_log_context, update_log_context
...
def _setup_logfile() -> "Path":
    ...
    """
    logfile = Path(settings.riven_dir / "logs" / "riven.log")
...
def get_logger(name: Optional[str] = None) -> "logging.Logger":
    ...
    """
    parent_logger = logging.getLogger("riven")
    if name:
