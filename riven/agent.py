...
from riven.agents.helpers import generate_step_id
from riven.constants import (
    CLI_WARNING_PREFIX,
    COMPOSIO_ENTITY_ENV_VAR_KEY,
    ERROR_MESSAGE_PREFIX,
    FIRST_MESSAGE_ATTEMPTS,
    FUNC_FAILED_HEARTBEAT_MESSAGE,
    riven_CORE_TOOL_MODULE_NAME,
    riven_MULTI_AGENT_TOOL_MODULE_NAME,
    LLM_MAX_TOKENS,
    READ_ONLY_BLOCK_EDIT_ERROR,
    REQ_HEARTBEAT_MESSAGE,
    SEND_MESSAGE_TOOL_NAME,
)
from riven.errors import ContextWindowExceededError
from riven.functions.ast_parsers import coerce_dict_args_by_annotations, get_function_annotations_from_source
from riven.functions.composio_helpers import execute_composio_action, generate_composio_action_from_func_name
from riven.functions.functions import get_function_from_module
from riven.helpers import ToolRulesSolver
from riven.helpers.composio_helpers import get_composio_api_key
from riven.helpers.datetime_helpers import get_utc_time
from riven.helpers.json_helpers import json_dumps, json_loads
from riven.helpers.message_helper import convert_message_creates_to_messages
from riven.interface import AgentInterface
from riven.llm_api.helpers import calculate_summarizer_cutoff, get_token_counts_for_messages, is_context_overflow_error
from riven.llm_api.llm_api_tools import create
from riven.llm_api.llm_client import LLMClient
from riven.local_llm.constants import INNER_THOUGHTS_KWARG
from riven.local_llm.utils import num_tokens_from_functions, num_tokens_from_messages
from riven.log import get_logger
from riven.memory import summarize_messages
...
from riven.schemas.agent import AgentState, AgentStepResponse, UpdateAgent
from riven.schemas.block import BlockUpdate
from riven.schemas.embedding_config import EmbeddingConfig
from riven.schemas.enums import MessageRole, ProviderType, StepStatus, ToolType
from riven.schemas.riven_message_content import ImageContent, TextContent
from riven.schemas.memory import ContextWindowOverview, Memory
from riven.schemas.message import Message, MessageCreate, ToolReturn
from riven.schemas.openai.chat_completion_response import ChatCompletionResponse, Message as ChatCompletionMessage, UsageStatistics
from riven.schemas.response_format import ResponseFormatType
from riven.schemas.tool import Tool
from riven.schemas.tool_execution_result import ToolExecutionResult
from riven.schemas.tool_rule import TerminalToolRule
...
from riven.services.helpers.agent_manager_helper import check_supports_structured_output
...
from riven.services.telemetry_manager import NoopTelemetryManager, TelemetryManager
from riven.services.tool_executor.tool_execution_sandbox import ToolExecutionSandbox
from riven.services.tool_manager import ToolManager
from riven.settings import model_settings, settings, summarizer_settings
from riven.streaming_interface import StreamingRefreshCLIInterface
from riven.system import get_heartbeat, get_token_limit_warning, package_function_response, package_summarize_message, package_user_message
from riven.utils import count_tokens, get_friendly_error_msg, get_tool_call_id, log_telemetry, parse_json, validate_function_response
...
class Agent(BaseAgent):
    ...
    def _handle_ai_response(
        ...
        if response_message.function_call or (response_message.tool_calls is not None and len(response_message.tool_calls) > 0):
            ...
            target_riven_tool = None
            for t in self.agent_state.tools:
                if t.name == function_name:
                    # This force refreshes the target_riven_tool from the database
                    # We only do this on name match to confirm that the agent state contains a specific tool with the right name
                    target_riven_tool = ToolManager().get_tool_by_name(tool_name=function_name, actor=self.user)
                    break

            if not target_riven_tool:
            ...
            try:
                ...
                log_event(
                    ...
                    attributes={
                        ...
                        "target_riven_tool": target_riven_tool.model_dump(),
                        **{f"function_args.{k}": v for k, v in function_args.items()},
                    },
                )

                tool_execution_result = self.execute_tool_and_persist_state(function_name, function_args, target_riven_tool)
                ...
                return_char_limit = target_riven_tool.return_char_limit
    ...
    def step(
        ...
        **kwargs,
        ...
        return rivenUsageStatistics(**total_usage.model_dump(), step_count=step_count, steps_messages=steps_messages)
    ...
    def execute_tool_and_persist_state(self, function_name: str, function_args: dict, target_riven_tool: Tool) -> ToolExecutionResult:
        """
        ...
        try:
            if target_riven_tool.tool_type == ToolType.riven_CORE:
                # base tools are allowed to access the `Agent` object and run on the database
                callable_func = get_function_from_module(riven_CORE_TOOL_MODULE_NAME, function_name)
                function_args["self"] = self  # need to attach self to arg since it's dynamically linked
                function_response = callable_func(**function_args)
            elif target_riven_tool.tool_type == ToolType.riven_MULTI_AGENT_CORE:
                callable_func = get_function_from_module(riven_MULTI_AGENT_TOOL_MODULE_NAME, function_name)
                function_args["self"] = self  # need to attach self to arg since it's dynamically linked
                function_response = callable_func(**function_args)
            elif target_riven_tool.tool_type == ToolType.riven_MEMORY_CORE or target_riven_tool.tool_type == ToolType.riven_SLEEPTIME_CORE:
                callable_func = get_function_from_module(riven_CORE_TOOL_MODULE_NAME, function_name)
            ...
            elif target_riven_tool.tool_type == ToolType.EXTERNAL_COMPOSIO:
                action_name = generate_composio_action_from_func_name(target_riven_tool.name)
            ...
            elif target_riven_tool.tool_type == ToolType.EXTERNAL_MCP:
                # Get the server name from the tool tag
                # TODO make a property instead?
                server_name = target_riven_tool.tags[0].split(":")[1]
                ...
                if function_name not in available_tool_names:
                    raise ValueError(
                        f"{function_name} is not available in MCP server {server_name}. Please check your `~/.riven/mcp_config.json` file."
                    )
                ...
                try:
                    ...
                    annotations = get_function_annotations_from_source(target_riven_tool.source_code, function_name)
                ...
                tool_execution_result = ToolExecutionSandbox(function_name, function_args, self.user, tool_object=target_riven_tool).run(
