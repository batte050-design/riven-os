...
from riven.config_file import apply_config_to_env
from riven.schemas.enums import SandboxType
from riven.services.summarizer.enums import SummarizationMode

apply_config_to_env()

# Define constants here to avoid circular import with riven.log
...
class SummarizerSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="riven_summarizer_", extra="ignore")
    ...
    # The amount of tokens before a system warning about upcoming truncation is sent to riven
...
class ModelSettings(BaseSettings):
    ...
    auto_mode_enabled: bool = Field(
        default=False,
        description="Enable auto mode model handles (i.e. riven/auto, riven/auto-fast) in model listing",
    )
...
# read pg_uri from ~/.riven/pg_uri or set to none, this is to support riven Desktop
default_pg_uri = None
...
if "--use-file-pg-uri" in sys.argv:
    try:
        with open(Path.home() / ".riven/pg_uri", "r") as f:
            default_pg_uri = f.read()
            print(f"Read pg_uri from ~/.riven/pg_uri: {default_pg_uri}")
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="riven_", extra="ignore")

    riven_dir: Optional[Path] = Field(Path.home() / ".riven", alias="riven_DIR")
    ...
    # Example:
    #   riven_OBJECT_STORE_URI="gs://my-bucket/repository?project=my-gcp-project"
    object_store_uri: str | None = Field(
        default=None,
        validation_alias=AliasChoices("riven_OBJECT_STORE_URI"),
    ...
    object_store_project: str | None = Field(
        default=None,
        validation_alias=AliasChoices("riven_OBJECT_STORE_PROJECT"),
    ...
    memfs_service_url: str | None = Field(
        default=None,
        validation_alias=AliasChoices("riven_MEMFS_SERVICE_URL"),
    ...
    clickhouse_endpoint: str | None = Field(
        default=None,
        validation_alias=AliasChoices("CLICKHOUSE_ENDPOINT", "riven_clickhouse_endpoint"),
        description="ClickHouse endpoint URL",
    )
    clickhouse_database: str | None = Field(
        default="otel",
        validation_alias=AliasChoices("CLICKHOUSE_DATABASE", "riven_clickhouse_database"),
        description="ClickHouse database name",
    )
    clickhouse_username: str | None = Field(
        default="default",
        validation_alias=AliasChoices("CLICKHOUSE_USERNAME", "riven_clickhouse_username"),
        description="ClickHouse username",
    )
    clickhouse_password: str | None = Field(
        default=None,
        validation_alias=AliasChoices("CLICKHOUSE_PASSWORD", "riven_clickhouse_password"),
    ...
    # riven client settings for tool execution
    default_base_url: str = Field(default="http://localhost:8283", description="Default base URL for riven client in tool execution")
    default_token: Optional[str] = Field(default=None, description="Default token for riven client in tool execution")

    # enabling riven_agent_v1 architecture
    use_riven_v1_agent: bool = False
    ...
    def riven_pg_uri(self) -> str:
        if self.pg_uri:
            ...
            return "postgresql+pg8000://riven:riven@localhost:5432/riven"

    # add this property to avoid being returned the default
    # reference: https://github.com/riven-ai/riven/issues/1362
    @property
    def riven_pg_uri_no_default(self) -> str:
        if self.pg_uri:
            return self.pg_uri
        elif self.pg_db and self.pg_user and self.pg_password and self.pg_host and self.pg_port:
            return f"postgresql+pg8000://{self.pg_user}:{self.pg_password}@{self.pg_host}:{self.pg_port}/{self.pg_db}"
    ...
    def database_engine(self) -> DatabaseChoice:
        return DatabaseChoice.POSTGRES if self.riven_pg_uri_no_default else DatabaseChoice.SQLITE
...
class TestSettings(Settings):
    model_config = SettingsConfigDict(env_prefix="riven_test_", extra="ignore")

    riven_dir: Path | None = Field(Path.home() / ".riven/test", alias="riven_TEST_DIR")


class LogSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="riven_logging_", extra="ignore")
    ...
    riven_log_path: Path | None = Field(Path.home() / ".riven" / "logs" / "riven.log")
...
class TelemetrySettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="riven_telemetry_", extra="ignore")
    ...
    datadog_service_name: str = Field(default="riven-server", description="Service name for Datadog profiling.")
...
class ReadinessSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="riven_readiness_", extra="ignore")
