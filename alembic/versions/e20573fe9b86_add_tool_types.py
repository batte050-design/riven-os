...
from riven.constants import BASE_MEMORY_TOOLS, BASE_TOOLS
...
def upgrade() -> None:
    ...
    if not settings.riven_pg_uri_no_default:
        return
    ...
    # Define the list of riven core tools
    riven_core_value = ToolType.riven_CORE.value
    riven_memory_core_value = ToolType.riven_MEMORY_CORE.value
    custom_value = ToolType.CUSTOM.value

    # Update tool_type for riven core tools
    op.execute(
        f"""
        UPDATE tools
        SET tool_type = '{riven_core_value}'
    ...
    op.execute(
        f"""
        UPDATE tools
        SET tool_type = '{riven_memory_core_value}'
